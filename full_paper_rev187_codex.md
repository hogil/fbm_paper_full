...
**Abstract** - **Failbit Map은 EDS(Electrical Die Sorting) 검사 결과를 wafer 좌표에 남기는 자료로, 불량의 위치와 형태를 함께 보여 준다. 그러나 실제 분석에서는 chip별 measure value와 제한된 map 조회에 의존하는 경우가 많아 반복 패턴이나 신규 failure를 늦게 발견할 수 있다. 여기서는 Failbit Map 이미지, chip 좌표, chip measure value, 모델 출력, review label을 같은 wafer 좌표계에 맞춰 저장하고, 이를 Known 분류, Unknown 후보 압축, chip-level multi-label 검증에 함께 사용했다. Known fail은 ConvNeXtV2-Base와 저신뢰 표본 한정 ROI(Region of Interest)-YOLO(You Only Look Once) 2-stage로 field validation weighted F1 0.95를 확인했다. Unknown fail은 대조학습 embedding과 밀도 군집화로 운영 wafer를 13개 후보 group으로 줄였고, 이 중 7개가 현업 검토에서 신규 failure로 확인되었다. 이 13→7은 분류 정밀도가 아니라 field review 후보 압축 결과다. chip 수준에서는 single-failure chip만으로 2-combo를 학습하기 위해 FCM-PM(Full-Cover CutMix + Pair Mask)을 설계했으며, controlled synthetic benchmark에서 micro-F1(bit-F1) 0.9927과 false alarm 0.00%를 얻었다. 각 결과는 field validation, field review, generated development, controlled synthetic benchmark로 분리해 해석했다. 따라서 Failbit Map은 한 번 보고 끝나는 그림이 아니라 review, label feedback, retraining에 다시 쓰이는 좌표 보존 자료가 된다.**
...
![DRAM cell에서 좌표 보존 Failbit Map까지](figures/full_paper_rev187/fig01_dram_to_fbm.png)
...
출발점은 두 가지였다. Failbit Map을 대량으로 만들고 볼 수 있어야 했고, 사람이 정한 임계값만으로는 신규 패턴을 놓칠 수 있었다. 그래서 raw log를 좌표 보존 이미지와 chip annotation으로 바꾸고, 그 위에 Known 분류와 Unknown 후보 압축을 연결했다. chip 수준에서는 known chip coordinate를 쓰는 object-id map과 single 불량 chip만으로 2-combo를 학습하는 FCM-PM을 별도로 두었다. 현업 review 화면에서는 검색, map view, chip overlay, measure 확인, label 관리가 이어진다. 결과는 operation impact, field validation, field review, generated development, controlled synthetic benchmark로 나누어 읽었고 서로 합산하지 않았다. 전체 흐름은 데이터 적재, 모델 분석, 현업 review의 세 단계로 정리된다.

중요한 점은 모델을 여러 개 붙였다는 사실이 아니다. 이미지, chip 좌표, 계측값, review 결과가 같은 wafer 좌표에 남도록 만든 것이 핵심이다. 이 좌표계가 유지되기 때문에 Known 분류, Unknown 후보 압축, chip-level 검증을 같은 자료 위에서 연결할 수 있었다. categorical object-id를 자연 해상도 one-hot으로 인코딩하면 fixed chip coordinate라는 반도체 prior 덕분에 localization 부담이 줄고, class별 공간 패턴이 남는다(측정 근거는 2.2.1). 이 결과는 모델 용량 우열이 아니라 입력 표현의 효과로 해석한다. 또 정답 2-combo 라벨이 없는 조건에서는 FCM-PM으로 단일 불량 chip에서 중복 불량 학습 신호를 만들었다. Pair Mask가 합성 background의 false alarm을 줄이는 원인이라는 점은 ablation으로 확인하였다(2.2.3). object-id map은 미양산 generated-chip development result다(2.2.1).
...
![좌표 보존 분석 파이프라인](figures/full_paper_rev187/fig02_pipeline_architecture.png)
...
![ROI-YOLO 2-stage 검출](figures/full_paper_rev187/fig03_roi_yolo.png)
...
![object-id map 생성 흐름](figures/full_paper_rev187/fig04_objid_flow.png)
...
![chip 라벨 공간 예시](figures/full_paper_rev187/fig05_chip_label_space.png)
...
![FCM-PM augmentation](figures/full_paper_rev187/fig06_fcm_pm.png)
...
![val_margin selection and NB-reject sidecar](figures/full_paper_rev187/fig07_probability_control.png)
...
아래 결과는 검증 단계가 서로 다르다. 그래서 각 수치는 Table 3의 scope 안에서 읽어야 한다. 운영 효과는 viewer/data pipeline이 현업 검토 방식을 바꾼 결과이고, 모델 metric은 각 모델과 benchmark의 성능 지표다. 두 축은 합산하거나 같은 classifier metric처럼 비교하지 않는다.
...
![raw와 object-id map 구분력 비교](figures/full_paper_rev187/fig_objid_compare.png)
...
![운영 web app(map viewer) 화면](figures/full_paper_rev187/fig09_webapp_viewer.png)
...
이번 작업에서는 wafer 이미지, chip 좌표, measure value, 모델 출력, 검토 라벨을 같은 좌표계에 묶어 Failbit Map을 반복 검토 가능한 자료로 만들었다. Known 경로에서는 ConvNeXtV2와 선택적 ROI-YOLO로 field validation weighted F1 0.95를 얻었고, object-id map은 known chip coordinate로 localization을 줄이는 generated development extension으로 두었다. Unknown 경로에서는 라벨 없는 운영 wafer를 13개 후보군으로 압축했고, 그중 7개 신규 불량 group이 field review에서 확인되었다. chip 수준에서는 FCM-PM이 단일 불량 chip만으로 2-combo 학습 신호를 만들고 Pair Mask로 false alarm을 억제하여 controlled synthetic benchmark에서 bit-F1 0.9927과 FAR 0.00%를 보였다. 운영 측면에서는 viewer/data pipeline이 약 48매 단위 조회를 일 약 2만 wafer 누적 비교로 확장했으며, Flash YE 확산 요청은 별도 적용 검토 범위로 두었다.
