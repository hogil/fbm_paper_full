...
![DRAM cell에서 좌표 보존 Failbit Map까지](figures/full_paper_rev205/fig01_dram_to_fbm.png)
...
출발점은 현업에서 반복해서 부딪힌 두 문제였다. Failbit Map을 대량으로 만들고 볼 수 있어야 했고, 사람이 정한 임계값만으로는 신규 pattern을 놓칠 수 있었다. 실제 작업은 raw log를 palette PNG와 chip annotation으로 바꾸는 데서 시작했다. FTN, QTN, BIN measure는 같은 chip 좌표에 붙이고, 모델 출력과 review label도 wafer ID와 chip 위치로 다시 찾을 수 있게 했다. 이 자료 위에서 등록 failure를 판정하고, 라벨 없는 신규 후보는 검토군으로 정리했다. chip 수준에서는 이미 알고 있는 chip 좌표를 쓰는 object-id map과 single 불량 chip만으로 2-combo를 학습하는 FCM-PM을 별도 경로로 두었다. 담당자는 review 화면에서 wafer를 검색하고, map과 chip overlay를 보며, measure와 라벨을 같은 자리에서 확인한다. 운영에서 줄어든 일과 모델 평가에서 얻은 숫자는 같은 종류가 아니다. 그래서 Known F1, Unknown 13→7, object-id map, FCM-PM, 운영 효과는 Table 3에서 출처를 따로 표시했고 한 점수처럼 더하지 않았다.
...
![좌표 보존 분석 파이프라인](figures/full_paper_rev205/fig02_pipeline_architecture.png)
...
![ROI-YOLO 2-stage 검출](figures/full_paper_rev205/fig03_roi_yolo.png)
...
![object-id map 생성 흐름](figures/full_paper_rev205/fig04_objid_flow.png)
...
![chip 라벨 공간 예시](figures/full_paper_rev205/fig05_chip_label_space.png)
...
![FCM-PM augmentation](figures/full_paper_rev205/fig06_fcm_pm.png)
...
![val_margin selection and NB-reject sidecar](figures/full_paper_rev205/fig07_probability_control.png)
...
![raw와 object-id map 구분력 비교](figures/full_paper_rev205/fig_objid_compare.png)
...
![운영 web app(map viewer) 화면](figures/full_paper_rev205/fig09_webapp_viewer.png)
