...
![DRAM cell에서 좌표 보존 Failbit Map까지](figures/full_paper_rev190/fig01_dram_to_fbm.png)
...
출발점은 현업에서 반복해서 부딪힌 두 문제였다. Failbit Map을 대량으로 만들고 볼 수 있어야 했고, 사람이 정한 임계값만으로는 신규 pattern을 놓칠 수 있었다. 그래서 raw log를 좌표 보존 이미지와 chip annotation으로 바꾸고, 그 위에 Known 분류와 Unknown 후보 압축을 연결했다. chip 수준에서는 이미 알고 있는 chip 좌표를 쓰는 object-id map과, single 불량 chip만으로 2-combo를 학습하는 FCM-PM을 별도 경로로 두었다. 현업 review 화면에서는 검색, map view, chip overlay, measure 확인, label 관리가 이어진다. 결과는 operation impact, field validation, field review, generated development, controlled synthetic benchmark로 나누어 읽었고, 서로 다른 범위의 수치를 하나의 metric처럼 합산하지 않았다.

중요한 점은 모델을 여러 개 붙였다는 사실이 아니다. 이미지, chip 좌표, 계측값, review 결과가 같은 wafer 좌표에 남도록 만든 것이 핵심이다. 이 좌표계가 유지되기 때문에 Known 분류, Unknown 후보 압축, chip-level 검증을 같은 자료 위에서 연결할 수 있었다. categorical object-id를 자연 해상도 one-hot으로 인코딩하면 fixed chip coordinate라는 반도체 prior 덕분에 localization 부담이 줄고, class별 공간 패턴이 남는다(측정 근거는 2.2.1). 이 결과는 모델 용량 우열이 아니라 입력 표현의 효과로 해석한다. 정답 2-combo 라벨이 없는 조건에서는 FCM-PM으로 단일 불량 chip에서 중복 불량 학습 신호를 만들었다. Pair Mask가 합성 background의 false alarm을 줄이는 원인이라는 점은 ablation으로 확인하였다(2.2.3). object-id map은 양산 적용 성과가 아니라 생성 데이터 기준 후속 개발 결과다(2.2.1).
...
![좌표 보존 분석 파이프라인](figures/full_paper_rev190/fig02_pipeline_architecture.png)
...
![ROI-YOLO 2-stage 검출](figures/full_paper_rev190/fig03_roi_yolo.png)
...
![object-id map 생성 흐름](figures/full_paper_rev190/fig04_objid_flow.png)
...
![chip 라벨 공간 예시](figures/full_paper_rev190/fig05_chip_label_space.png)
...
![FCM-PM augmentation](figures/full_paper_rev190/fig06_fcm_pm.png)
...
![val_margin selection and NB-reject sidecar](figures/full_paper_rev190/fig07_probability_control.png)
...
![raw와 object-id map 구분력 비교](figures/full_paper_rev190/fig_objid_compare.png)
...
![운영 web app(map viewer) 화면](figures/full_paper_rev190/fig09_webapp_viewer.png)
...
이 논문의 핵심은 개별 모델의 성능 경쟁이 아니다. wafer 이미지, chip 좌표, measure value, 모델 출력, 검토 라벨을 하나의 좌표계에 묶고, 그 위에서 Known 판정, Unknown 후보 압축, chip multi-label 검증이 이어지도록 만든 것이다. wafer의 공간 의미를 훼손하지 않는다는 원칙은 palette 표현, flip 배제, structured sampling, one-hot 인코딩으로 이어졌다. Known 경로는 ConvNeXtV2와 선택적 ROI-YOLO로 field validation weighted F1 0.95를 확인했고, Unknown 경로는 라벨 없는 운영 wafer에서 신규 failure 후보를 13개 group으로 줄여 7개 신규 failure를 확인했다. FCM-PM은 single 불량 chip만으로 2-combo 학습 신호를 만들어 controlled synthetic benchmark에서 bit-F1 0.9927과 FAR 0.00%를 보였다. viewer/data pipeline은 약 48매 단위 조회를 일 약 2만 wafer 누적 비교로 확장했으며, object-id map과 chip model serving은 별도 검증 단계로 관리한다. 결국 Failbit Map은 한 번 보고 끝나는 이미지가 아니라, 담당자가 다시 찾고 라벨을 쌓아 다음 학습에 되돌릴 수 있는 운영 자산이 된다.
