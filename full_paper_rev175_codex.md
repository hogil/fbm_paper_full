...
![DRAM cell에서 좌표 보존 Failbit Map까지](figures/full_paper_rev175/fig01_dram_to_fbm.png)
...
![좌표 보존 분석 파이프라인](figures/full_paper_rev175/fig02_pipeline_architecture.png)
...
![ROI-YOLO 2-stage 검출](figures/full_paper_rev175/fig03_roi_yolo.png)
...
![object-id map 생성 흐름](figures/full_paper_rev175/fig04_objid_flow.png)
...
![chip 라벨 공간 예시](figures/full_paper_rev175/fig05_chip_label_space.png)
...
![FCM-PM augmentation](figures/full_paper_rev175/fig06_fcm_pm.png)
...
![val_margin selection and NB-reject sidecar](figures/full_paper_rev175/fig07_probability_control.png)
...
Table 7은 recipe별 기여를 분리한다. BCE+label smoothing은 bit-F1 0.1093·FAR 99.47%였고, Focal[15]과 ASL[16]도 FAR 억제가 불충분했다. 단순 CutMix는 bit-F1을 높였지만 FAR 42.05%가 남았고, Pair Mask를 더해야 24.62%까지 낮아졌다. 채택 단일 모델은 FCM-M 행이며 bit-F1 0.9927, 2-combo 0.9871, Total FAR 0.00%에 도달했다. Ensemble은 upper-bound, KD는 compression candidate다. 약어는 CM=CutMix, C+PM=CutMix+Pair Mask, FCM-F=FCM-PM+val_f1, FCM-M=FCM-PM+val_margin, Ens.=ensemble이다.
...
![운영 web app(map viewer) 화면](figures/full_paper_rev175/fig09_webapp_viewer.png)
