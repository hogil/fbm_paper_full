...
![DRAM cell에서 좌표 보존 Failbit Map까지](figures/full_paper_rev189/fig01_dram_to_fbm.png)
...
![좌표 보존 분석 파이프라인](figures/full_paper_rev189/fig02_pipeline_architecture.png)
...
![ROI-YOLO 2-stage 검출](figures/full_paper_rev189/fig03_roi_yolo.png)
...
![object-id map 생성 흐름](figures/full_paper_rev189/fig04_objid_flow.png)
...
object-id map의 목적은 더 작은 모델로 detector를 이기는 것이 아니다. ROI-YOLO는 원래 위치를 모르는 물체를 찾는 detector라서 후보 box를 찾고, box를 보정하고, NMS로 중복 후보를 지운다. 그러나 wafer의 chip 위치는 제품 layout으로 이미 정해져 있고, 데이터 생성 단계에서 chip 좌표도 확보된다. 이 점을 쓰면 Stage 2를 detection 문제가 아니라 chip별 fixed-crop classification 문제로 바꿀 수 있다. 각 chip을 한 번 잘라 chip-CNN이 local pattern을 분류하고, 그 class id를 원래 wafer grid에 다시 배치하면 object-id map이 된다. full wafer image에서는 작은 chip failure가 희석되지만, chip crop에서는 local morphology가 직접 보인다. 따라서 object-id map의 결과는 1.16M chip-CNN이 88M backbone을 단순히 이겼다는 뜻이 아니다. fixed chip coordinate를 활용해 localization을 줄이고, categorical spatial representation을 만든 효과로 해석해야 한다.
...
![chip 라벨 공간 예시](figures/full_paper_rev189/fig05_chip_label_space.png)
...
2-combo chip label을 실제로 충분히 모으기는 어렵다. 그래서 single-failure chip source를 그대로 활용해 두 failure가 함께 있는 상황을 만들었다. FCM-PM은 두 single-failure chip을 full-cover 방식으로 합쳐 A+B mixed chip을 만들고, 동시에 A-only와 B-only masked view를 함께 학습한다(Figure 6). mixed view는 두 failure가 함께 있을 때의 positive signal을 주고, masked view는 합성 과정에서 생긴 background가 false-positive signal로 학습되는 것을 막는다. 그래서 FCM-PM은 단순한 augmentation이 아니라 weak positive를 보강하면서 negative tail을 제어하는 학습 구조로 해석된다. false alarm rate(FAR)는 negative 중 오검출 비율(FP_neg/N_neg)로 계산하고, checkpoint는 validation에서 val_margin = mean(p_pos) - max(p_neg)이 가장 큰 epoch로 선택한다.
...
![FCM-PM augmentation](figures/full_paper_rev189/fig06_fcm_pm.png)
...
![val_margin selection and NB-reject sidecar](figures/full_paper_rev189/fig07_probability_control.png)
...
![raw와 object-id map 구분력 비교](figures/full_paper_rev189/fig_objid_compare.png)
...
![운영 web app(map viewer) 화면](figures/full_paper_rev189/fig09_webapp_viewer.png)
...
핵심은 개별 모델의 성능 경쟁이 아니다. wafer 이미지, chip 좌표, measure value, 모델 출력, 검토 라벨을 하나의 좌표계에 묶고, 그 위에서 Known 판정, Unknown 후보 축소, chip multi-label 검증이 이어지도록 만든 것이다. Known 경로는 ConvNeXtV2와 선택적 ROI-YOLO로 field validation weighted F1 0.95를 얻었다. Unknown 경로는 라벨 없는 운영 wafer를 13개 후보 group으로 줄였고, 그중 7개 신규 불량 group이 field review에서 확인되었다. FCM-PM은 single 불량 chip만으로 2-combo 학습 신호를 만들어 controlled synthetic benchmark에서 bit-F1 0.9927과 FAR 0.00%를 보였다. viewer/data pipeline은 약 48매 단위 조회를 일 약 2만 wafer 누적 비교로 확장했으며, object-id map과 chip model serving은 별도 검증 단계로 관리한다. 결국 Failbit Map은 한 번 보고 끝나는 이미지가 아니라 담당자가 다시 찾고 라벨을 쌓아 다음 학습에 되돌릴 수 있는 자료가 된다.
