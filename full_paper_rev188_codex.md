...
**Abstract** - **Failbit Map은 wafer 위 failure 위치와 grade를 함께 남기는 진단 자료다. 그러나 현업 분석에서는 chip별 measure value와 제한된 map review가 따로 움직이는 경우가 많아 반복 pattern을 대량으로 비교하거나 신규 failure 후보를 빠르게 좁히기 어렵다. 이번 작업에서는 EDS(Electrical Die Sorting) raw log에서 만든 Failbit Map, chip 좌표, chip measure, 모델 출력, review label을 같은 wafer 좌표계에 맞췄다. 이 자료 위에서 Known fail은 ConvNeXtV2-Base와 저신뢰 표본 한정 ROI(Region of Interest)-YOLO(You Only Look Once) 2-stage로 field validation weighted F1 0.95를 얻었다. Unknown fail은 대조학습 embedding과 밀도 군집화로 운영 wafer를 13개 후보 group으로 줄였고, 현업 검토에서 7개 신규 failure group을 확인했다. chip 수준에서는 object-id map을 생성 데이터 기준 후속 개발 항목으로 두고, FCM-PM(Full-Cover CutMix + Pair Mask)으로 single-failure source만 이용한 2-combo controlled synthetic benchmark에서 micro-F1(bit-F1) 0.9927과 false alarm 0.00%를 얻었다. 각 결과는 field validation, field review, generated development, controlled synthetic benchmark로 나누어 해석한다. 따라서 Failbit Map은 한 번 보고 끝나는 그림이 아니라 담당자가 다시 찾고 라벨을 쌓아 다음 학습에 쓸 수 있는 좌표 보존 자료가 된다.**
...
![DRAM cell에서 좌표 보존 Failbit Map까지](figures/full_paper_rev188/fig01_dram_to_fbm.png)
...
![좌표 보존 분석 파이프라인](figures/full_paper_rev188/fig02_pipeline_architecture.png)
...
![ROI-YOLO 2-stage 검출](figures/full_paper_rev188/fig03_roi_yolo.png)
...
![object-id map 생성 흐름](figures/full_paper_rev188/fig04_objid_flow.png)
...
![chip 라벨 공간 예시](figures/full_paper_rev188/fig05_chip_label_space.png)
...
![FCM-PM augmentation](figures/full_paper_rev188/fig06_fcm_pm.png)
...
![val_margin selection and NB-reject sidecar](figures/full_paper_rev188/fig07_probability_control.png)
...
![raw와 object-id map 구분력 비교](figures/full_paper_rev188/fig_objid_compare.png)
...
![운영 web app(map viewer) 화면](figures/full_paper_rev188/fig09_webapp_viewer.png)
