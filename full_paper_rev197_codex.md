...
**Abstract** - **Failbit Map은 wafer 위 failure 위치와 grade를 함께 남기는 진단 자료다. 그러나 현업 분석에서는 chip별 measure value, map review, 라벨 기록이 따로 남는 경우가 많다. 담당자는 의심 wafer를 다시 찾고 map과 수치를 맞춰 보아야 하므로, 반복 pattern을 대량으로 비교하거나 신규 failure를 빠르게 가려내기 어렵다. 이번 작업에서는 EDS(Electrical Die Sorting) raw log에서 만든 Failbit Map, chip 좌표, chip measure, 모델 출력, review label을 같은 wafer 좌표계에 맞췄다. 이 좌표 보존 자산 위에서 등록 failure는 ConvNeXtV2-Base와 저신뢰 표본 한정 ROI(Region of Interest)-YOLO(You Only Look Once) 2-stage로 field validation weighted F1 0.95를 얻었다. 라벨이 없는 신규 패턴은 contrastive embedding과 HDBSCAN(Hierarchical Density-Based Spatial Clustering of Applications with Noise)으로 운영 wafer를 13개 검토 group으로 정리했고, 현업 검토에서 7개 신규 failure group을 확인했다. chip 수준에서는 object-id map을 생성 데이터 기준 후속 개발 항목으로 두고, FCM-PM(Full-Cover CutMix + Pair Mask)으로 single-failure source 기반 2-combo controlled synthetic benchmark에서 micro-F1(bit-F1) 0.9927과 false alarm 0.00%를 얻었다. 각 결과는 field validation, field review, generated development, controlled synthetic benchmark로 나누어 해석했다. 따라서 Failbit Map은 한 번 보고 끝나는 그림이 아니라, 담당자가 다시 찾고 chip 근거를 확인하며 라벨을 쌓아 다음 학습에 연결할 수 있는 좌표 보존 분석 자산이 된다.**
...
![DRAM cell에서 좌표 보존 Failbit Map까지](figures/full_paper_rev197/fig01_dram_to_fbm.png)
...
출발점은 현업에서 반복해서 부딪힌 두 문제였다. Failbit Map을 대량으로 만들고 볼 수 있어야 했고, 사람이 정한 임계값만으로는 신규 pattern을 놓칠 수 있었다. 실제 작업은 raw log를 palette PNG와 chip annotation으로 바꾸는 데서 시작했다. FTN, QTN, BIN measure는 같은 chip 좌표에 붙이고, 모델 출력과 review label도 wafer ID와 chip 위치로 다시 찾을 수 있게 했다. 이 자료 위에서 등록된 Known failure를 판정하고, label이 없는 Unknown failure는 검토 group으로 정리했다. chip 수준에서는 이미 알고 있는 chip 좌표를 쓰는 object-id map과 single 불량 chip만으로 2-combo를 학습하는 FCM-PM을 별도 경로로 두었다. 담당자는 review 화면에서 wafer를 검색하고, map과 chip overlay를 보며, measure와 라벨을 같은 자리에서 확인한다. 현업에서 줄어든 일과 모델 평가에서 얻은 숫자는 같은 종류가 아니다. 그래서 Known F1, Unknown 13→7, object-id map, FCM-PM, 운영 효과는 Table 3에서 출처를 따로 표시했고 한 점수처럼 더하지 않았다.
...
![좌표 보존 분석 파이프라인](figures/full_paper_rev197/fig02_pipeline_architecture.png)
...
![ROI-YOLO 2-stage 검출](figures/full_paper_rev197/fig03_roi_yolo.png)
...
![object-id map 생성 흐름](figures/full_paper_rev197/fig04_objid_flow.png)
...
![chip 라벨 공간 예시](figures/full_paper_rev197/fig05_chip_label_space.png)
...
![FCM-PM augmentation](figures/full_paper_rev197/fig06_fcm_pm.png)
...
![val_margin selection and NB-reject sidecar](figures/full_paper_rev197/fig07_probability_control.png)
...
결과에 앞서 각 수치의 산출 범위를 먼저 밝힌다. Known F1은 field validation 값이고, Unknown 13→7은 검토 대상 group을 13개로 정리한 field review 결과다. object-id map은 생성 데이터 기준 후속 개발값이며, FCM-PM은 통제된 합성 평가값이다. viewer/data pipeline의 운영 효과도 모델 성능 수치와 성격이 다르다. 그래서 Table 3에 각 수치의 출처를 먼저 표시하고, 운영 효과와 모델 성능을 한 점수처럼 더하지 않았다.
...
![raw와 object-id map 구분력 비교](figures/full_paper_rev197/fig_objid_compare.png)
...
운영 적용에서 대조 임베딩과 HDBSCAN(2.1.3)은 수천 장의 운영 wafer를 13개 검토 group으로 정리했고, 그중 7개가 전문가 검토에서 신규 불량으로 확인되었다. 실전 Unknown은 정답 라벨이 없고 양산 noise가 커 F1/ARI 같은 정량 점수가 성립하지 않는다. 따라서 13→7은 classifier precision이 아니라 담당자가 13개 group을 검토해 7개 신규 불량을 확인한 field review 결과로만 해석한다. Unknown embedding 경로의 측정값은 frozen encoder forward가 wafer당 약 14.3 ms, HDBSCAN 포함 end-to-end가 약 24 ms였으나, 이는 모델 경로의 개발과 검증 측정값이다. Table 3의 FBM generation/storage는 Cython 변환 속도와 palette 저장 효율만 요약하며, 이 값을 model serving throughput처럼 해석하지 않는다.

후속 개발값을 관리하기 위해 정답 라벨이 있는 별도 합성 평가셋에서 Local DenseCL, MoCo queue, NV-Retriever, NeCo를 단계적으로 비교하였다(Table 6). 이 표는 실전 13/7과 분리된 component-isolation 평가셋(per class 500, normal 2000)의 단일 run 값이다. 대조 head는 noise를 15.78%에서 0.00%로 낮췄고, Completeness는 0.9679로 높였다. 가장 큰 단일 감소는 MoCo queue에서 나타나 noise가 13.87%에서 9.45%로 줄었다. Completeness와 n_cluster는 generated-development recipe 관리 지표이며 field Unknown 13→7과 한 점수처럼 더하지 않는다.
...
구축한 viewer/data pipeline은 2025년부터 DRAM D1a~D1d 관련 제품 파트의 FBM review infrastructure로 사용된다. 담당자는 이 화면에서 wafer ID를 검색하고, map overlay와 chip measure를 확인한 뒤 review label을 남긴다. 여기서 사용 범위는 FBM 생성, 조회, overlay, label review를 지원하는 review infrastructure에 한정되며, Known/Unknown/chip model serving이나 object-id map 양산 적용 완료를 의미하지 않는다. 1시간 주기 자동 적재로 기존 도구의 한 번에 약 48매 열람이 일 약 2만 wafer 누적 비교로 확장되었다(Table 8). 운영에서는 13개 검토 group 중 7개가 신규 불량으로 확인되었고, 이 결과가 라벨과 재학습 자료로 다시 쌓였다(Figure 9). 이 전환의 운영 효과는 분석 공수 약 90% 절감, P3WN 제품 사례의 수율 약 0.02%p 개선, 메모리제조기술센터 내부 성과 인정 기준 약 123억 규모로 집계되고 인정되었다.[^3] 또한 Flash YE에서 본 분석 구조의 확산 요청이 접수되어, Flash/NAND 계열 bin map review와 chip multi-label 분석으로의 적용을 검토 중이다. 다만 운영 정량 성과는 DRAM viewer/data pipeline 기준으로 한정한다. 이 값들은 viewer/data pipeline의 운영 효과이며, Table 4~7의 모델 성능 수치와 한 점수처럼 더하지 않는다.

![운영 web app(map viewer) 화면](figures/full_paper_rev197/fig09_webapp_viewer.png)
...
| Unknown 신규 패턴 | 수동 산발 발견 | 13개 검토 group으로 정리 |
...
마지막까지 유지한 기준은 개별 모델의 성능 경쟁이 아니라 좌표를 잃지 않는 것이었다. wafer 이미지, chip 좌표, measure value, 모델 출력, 검토 라벨을 하나의 좌표계에 묶었고, 담당자는 같은 chip을 다시 열어 map과 수치, 라벨을 함께 볼 수 있게 했다. 이 원칙은 palette 표현, flip 배제, structured sampling, one-hot 인코딩으로 이어졌다. Known 경로는 ConvNeXtV2와 선택적 ROI-YOLO로 field validation weighted F1 0.95를 확인했고, Unknown 경로는 라벨 없는 운영 wafer를 13개 검토 group으로 정리해 7개 신규 failure를 확인했다. FCM-PM은 single 불량 chip만으로 2-combo 학습 신호를 만들어 controlled synthetic benchmark에서 bit-F1 0.9927과 FAR 0.00%를 보였다. viewer/data pipeline은 약 48매 단위 조회를 일 약 2만 wafer 누적 비교로 확장했으며, object-id map과 chip model serving은 별도 검증 단계로 관리한다. 결국 Failbit Map은 한 번 보고 끝나는 이미지가 아니라, 담당자가 wafer ID로 다시 열어 chip 근거를 확인하고 판단 라벨을 다음 학습 목록에 남길 수 있는 작업 기록이 된다.
