# Humanize and layout checklist - full_paper_rev201_codex
...
- [PASS] Table 3 layout: Item / Value / Scope wording을 더 줄였고, rendered page 5 하단에 통째로 배치되어 page split 없음.
...
- [PASS] 새 수치 추가 없음: rev201은 rev200의 claim/scope를 유지하고 Figure 1 caption 및 Table 3 row text만 압축한 layout polish revision.

Notes:
- `tools\verify_full_paper_package.py`는 legacy expected figure/table/reference count와 old Abstract-heading assumption 때문에 FAIL을 보고한다. 현재 rev201의 core checks는 A4, margins, header/footer, two columns, figure files, citation/reference consistency, hidden XML scan, PDF page count 9를 통과했다.
- `full_paper_rev201_codex.docx`는 Table 3이 빌드 결과만으로 page split 없이 들어간 제출용 Word 파일이다.
