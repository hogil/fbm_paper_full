...
| **[추가 생성 데이터, 개발 중]** | Unknown 후속 개발 지표 | 실전 운영 성과와 분리한 후속 benchmark 입니다. same-anchor defect-class capture 43/43 (capture ratio 1.000), ARI **0.859 ± 0.018**, Completeness **0.994**, Homogeneity **0.942**. 후처리 기준 ARI **0.868 ± 0.013** |
...
학습 및 평가 운영 측면에서는 작은 validation set 에서 val_f1 만으로 checkpoint 를 고르기 어려워, false-positive 위험을 함께 보기 위한 val_margin 기준을 도입했습니다. KD single student 는 1× inference cost 로 줄일 수 있는지 확인한 압축 실험이며, stress 평가에서 FAR 이 높게 나와 바로 운영 대상으로 보지는 않았습니다.
...
[FCM-PM 선택]
    ...
    - ablation 변형에서 negative false-positive 억제 실패를 확인해 FCM-PM 조합 유지
        ↓
[checkpoint 선택]
...
val_margin 은 `정답 class score 평균 - 오답 class score 최대값` 으로 정의했습니다. 작은 validation set 에서 val_f1 만으로 checkpoint 를 고르기 어려워, false-positive 위험을 함께 보기 위한 best-model 선택 신호로 사용했습니다.
...
| **[추가 생성 chip 데이터, PoC]** | FCM-PM ablation | Full-Cover Mixup 이 없거나 Pair Mask 구성이 빠진 변형은 negative false-positive 억제가 실패했습니다. 본문에서는 FCM-PM 조합 필요성을 보여주는 보조 근거로 정리했습니다. |
...
| FCM-PM ablation | Full-Cover Mixup 또는 Pair Mask 구성의 필요성 확인 | 일부 변형에서 Total FAR **100.00%** 로 실패 |
...
[추가 생성 chip 데이터, PoC] 세부 ablation 수치는 실험 로그로 분리하고, 본문에서는 FCM-PM 조합 필요성을 보여주는 보조 근거로만 정리했습니다.
...
생성 데이터 점검 과정에서는 일시적인 validation 변동에 흔들리지 않도록 평가 로그를 별도로 확인했습니다.
