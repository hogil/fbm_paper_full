...
softmax 대신 sigmoid multi-label head 를 쓴 이유는 single failure 와 2-combo failure 가 mutually exclusive 가 아니기 때문입니다. FCM-PM 합성은 포함된 failure label 의 union 을 target 으로 두고, Pair Mask 는 B 영역이 mask 처리된 추가 augmentation chip 을 같이 학습시켜 single class 단독 패턴을 유지하면서 background 영역이 failure 로 오학습되지 않게 합니다. FAR 는 Normal / Invalid / OOD negative set 에서 별도로 계산해 운영 false-positive 위험을 직접 보게 했습니다.
