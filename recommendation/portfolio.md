...
현업 측면은 현업 환경에서 중복 failure label 확보가 어려운데 실제로 중복 failure 가 발생하고, Normal / Invalid / OOD 까지 있어 noise 도 많은 상황이었습니다. 기술적 측면은 최대한 현업과 유사한 환경을 구성하기 위해 train 은 single 만으로 진행해야 해서 2-combo prediction 성능 향상이 어려운 조건이었고, 평가 측면은 작은 validation set 의 best epoch plateau 와 OOD / negative false positive 억제를 같이 잡아야 했으며, 운영 측면은 압축 후보의 1× inference cost 제약 안에서 답을 만들어야 했기 때문에, 데이터 생성 설계 / loss 제어 / best-model selection / 추론 단계 보강을 단일 학습 구조 안에서 함께 풀어야 했습니다.
...
현업 EDS Failbit Map 에서 관찰되는 single failure pattern 4 종 (bank_boundary / fork / scratch / scratch_rot) 의 공간 분포와 grade 강도를 그대로 재현하기 위해, class 별 alpha mask 함수와 intensity tier 별 grade 0-7 categorical sampling 분포 (bank_boundary 는 3-zone, fork / scratch / scratch_rot 는 2-stage) 를 도메인 통계에 맞춰 직접 설계해 chip 을 생성했습니다. train 은 single 4 class 만 두고, eval 은 single 4 종 + 2-combo 6 종에 더해 현업 운영에서 함께 들어오는 Normal / Invalid / OOD chip 까지 포함해 약 3,850 chip 의 평가셋을 구성했습니다.
