...
FCM-PM 은 chip grid 를 complementary partition 으로 나누어 두 single failure chip 을 조합합니다. FCM mixed 는 label union 으로 multi-label 신호를 만들고, Pair Mask 는 한쪽 결함만 남긴 A-only / B-only augmentation 으로 단독 패턴 유지와 background false-positive 억제를 함께 잡습니다.
