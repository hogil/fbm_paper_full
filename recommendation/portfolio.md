...
- **기술 지표**
  ...
  **Unknown contrastive 구성요소 성능표 (per class 500, normal 2000) [후속 개발 / 추가 생성 데이터, 개발 중]**
  ...
  본 표는 §[Unknown 추가 생성 데이터, 개발 중] 분리 원칙에 따라 P1 Unknown 실전 운영 검출력과 분리한 후속 개발 / metric 관리용 synthetic benchmark 결과입니다.
...
**P3 결론**: 본인 trend 판정 경험으로 만든 합성 trend sample **약 7,000개** + Binary F1 **0.9967** 학습 / 검증 완료 (현업 데이터 적용 직전). 실전 abnormal log 기반 generator 재보정과 type 세분화 분류기는 후속 단계.
