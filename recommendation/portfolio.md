...
**P1 핵심 수치 요약**: 파이프라인 Cython **100배** + palette **75%** + 일 약 **2만 wafer / 1시간 주기** 처리, Known 2-stage weighted F1 **0.95** + Unknown 13 후보 중 **7건 실제 불량 검증**. 현재 Failbit Map 적재 파이프라인과 운영 뷰어는 양산 라인에서 사용 중이며, Known / Unknown AI 모델은 실전 현업 데이터 검증을 마치고 전수 자동 추론 적용을 준비 중입니다. chip-CNN object-id map 과 Unknown synthetic benchmark 는 후속 개발 트랙.
...
ROI-YOLO 는 chip 위치를 모르는 상태에서 bounding box, 좌표, class, NMS 를 함께 풀어야 하므로 메모리와 추론 비용 부담이 있습니다. 반면 chip-CNN object-id map 은 이미 생성된 chip 좌표를 활용해 crop classification 만 수행하므로 Stage 2 대체 후보로 개발 중입니다 (val_f1 **0.9946** / test_f1 0.9872 / 5-seed 평균 0.9838 ± 0.0092). 양산 적용 여부는 validation 검증 후 운영 절차에 따라 결정합니다.
...
- **기술 지표**
  - **[실전 현업 데이터]** Known 2-stage weighted F1 **0.95** (16 class / 1,500 labeled samples / 4:1 stratified split). 단계별 ladder 0.78 → 0.95 의 4단계 효과 분해는 §[최적화] 참고. label 부족 조건에서 backbone 교체와 cascade 결합이 결정적이었습니다.
  ...
  - **[Unknown 추가 생성 데이터, 개발 중]** 현업 데이터와 최대한 유사하게 구성한 별도 평가셋에서 신규 class **capture 1.000 (38/38 발견)** / **noise 0.00%** / **Completeness 0.9938** / **Silhouette 0.781** 까지 측정했습니다 (실전 운영 13/7 검증과 분리한 후속 개발 보조 지표).
...
**P2 핵심 수치 요약**: 대표 성과는 FCM-PM + val_margin 단일 모델의 bit_F1 **0.9943** / Total FAR **0.00%** 입니다. bit-level majority voting ensemble 과 Knowledge Distillation 은 운영 안정성 및 추론 생산성 검토를 위한 후속 항목입니다.
...
본인은 이를 Full-Cover Mixup 과 Pair Mask 로 나누어 풀었습니다. Full-Cover Mixup 은 chip 을 grid 로 나누어 두 single failure chip 의 영역을 전체 chip 범위에 걸쳐 조합하므로, 일반 CutMix 처럼 failure signal 이 특정 직사각형 crop 에서 잘리는 문제를 줄입니다. Pair Mask 는 합성 과정에서 다른 chip 영역이 붙은 위치를 mask 처리한 paired augmentation 을 함께 학습시켜, single failure 단독 패턴을 유지하고 background 가 failure 로 오학습되는 경로를 차단합니다. Pair Mask 제거 시 Total FAR 이 **100%** 까지 상승해, background loss 분리가 false-positive 억제의 핵심임을 확인했습니다.
...
이후 운영 환경 약 80% Normal 분포에 대응해 max-prob < 0.55 입력을 Normal 로 강제하는 threshold gate 를 적용했고, positive target 0.85 / negative target 0.15 설정으로 negative 측 보수성을 확보했습니다. bit-level majority voting ensemble 은 현업 분포 shift 대비 안정성 검토, Knowledge Distillation single student 는 1× 추론 비용 압축 후보로 분리했습니다.
...
Grade 0-7 양자화 chip 이미지는 값의 의미 보존이 중요하므로 pixel 보간 방식보다 원값을 유지하는 CutMix 계열이 적합하다고 판단했습니다 (자문: 연세대학교 인공지능학과 박은병 교수). 다만 일반 CutMix 는 failure signal 이 잘리거나 background 가 failure 로 오학습되는 문제가 있어, chip 전체 grid 를 cover 하는 Full-Cover Mixup 과 paired augmentation 인 Pair Mask 를 결합했습니다. Pair Mask 제거 시 Total FAR 이 **100%** 까지 상승해, background loss 분리가 false-positive 억제의 핵심임을 확인했습니다.
...
FCM-PM 은 chip 을 격자로 나눠 두 그룹 (P / ~P) 으로 쪼개고, 한쪽 chip 이 P 를 차지하면 다른 chip 은 남은 ~P 를 차지하도록 complementary 하게 배치합니다. 결과적으로 FCM mixed (A label) 와 FCM mixed (B label) 의 같은 chip pixel 을 합치면 원본 chip 이 그대로 복원됩니다 (mixed_A 의 A 영역 P + mixed_B 의 A 영역 ~P = full A). Pair Mask 는 그 합성 chip 에서 한쪽 결함만 남기고 나머지 자리를 흰색으로 가린 augmentation 으로, A-only 와 B-only 의 가림 위치가 P / ~P 로 서로 달라 single failure 의 단독 패턴 학습과 흰 배경 false-positive 억제를 같이 잡습니다.
...
운영 환경 약 80% Normal 분포에 대응해 max-prob < 0.55 입력을 Normal 로 강제하는 threshold gate 로 false-positive 를 추가 억제했습니다. bit-level majority voting ensemble 은 현업 분포 shift 에 대비한 안정성 보조 실험이며, Knowledge Distillation single student 는 ensemble 판단을 1× 추론 비용으로 압축하기 위한 후속 후보입니다. 두 항목 모두 대표 성과가 아니라 운영 안정성 / 생산성 검토 항목으로 분리했고, 정량 비교는 §[구현 성과] ablation 표 row 8~10 에 둡니다.
