...
본인은 현업 경험을 바탕으로 분석 담당자로부터 Failbit Map 분석법을 교육받고 설계했습니다. 본 과제에서 raw log → wafer 이미지 변환 / 저장 / 조회 파이프라인 (fail-map), 사내 운영 뷰어 web app, Known 2-stage 분류, Unknown self-supervised 검출, 후속 chip-CNN object-id map 보정 구조까지 설계 / 구현 / 검증을 했습니다. AI 모델을 구동하기 위한 GPU 도 수요 조사를 통해 확보했습니다 (GPU 할당 **2026년 9월**).
...
**P2 핵심 요약**: 운영에서는 2개 이상의 중복 failure 가 발생하지만 label 확보가 어려워, train 은 현업 EDS Failbit Map 의 4 종 single failure pattern 과 최대한 비슷하게 single chip 만 생성하고, eval 은 single 4 + 2-combo 6 종에 Normal / Invalid / OOD 까지 포함했습니다. single 만으로 2-combo prediction 성능을 끌어올리기 위해 FCM-PM (Full-Cover CutMix + Pair Mask) 을 직접 설계하고, 그 위에 val_margin best-model selection / Temperature Scaling / max-prob gate / ensemble / KD 까지 단일 흐름으로 이어 단일 모델 **bit_F1 0.9927 / Total FAR 0.00%** 를 달성했습니다.
...
현업 측면은 현업 환경에서 중복 failure label 확보가 어려운데 실제로 중복 failure 가 발생하고, Normal / Invalid / OOD 까지 있어 noise 도 많은 상황이었습니다. 기술적 측면은 최대한 현업과 유사한 환경을 구성하기 위해 train 은 single 만으로 진행해야 해서 eval prediction 성능 향상이 어려운 조건이었습니다.
...
현업 EDS Failbit Map 에서 관찰되는 single failure pattern 4 종 (bank_boundary / fork / scratch / scratch_rot) 의 공간 분포와 grade 강도를 최대한 비슷하게 재현하기 위해, class 별 alpha mask 함수와 intensity tier 별 grade 0-7 categorical sampling 분포를 통해 생성했습니다. train 은 single 4 class 만 두고, eval 은 single 4 종 + 2-combo 6 종에 더해 현업 운영에서 함께 들어오는 Normal / Invalid / OOD chip 까지 포함해 약 3,850 chip 의 평가셋을 구성했습니다.
