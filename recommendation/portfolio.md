...
위 production 구성과 분리해, MoCo Queue / NV-Retriever / NeCo 같은 SOTA recipe ablation 은 synthetic benchmark (per class 500, normal 2000) 별도 트랙에서 진행 중입니다 (§구현 성과 표 참고).
...
학습 코드 (`_train_chip_variant.py::complement` 모드, `n_groups=2`, pair-fill white) 를 실제 chip 이미지에 적용한 결과입니다. recipe 자체는 per-class 50 평가에서 가장 많이 검증된 group2 SOTA (iter26F: bit_F1 0.9953 / Total FAR 0.00%) 와 동일하고, 셀 분포가 한눈에 보이도록 GRID 만 4 로 줄여 그렸습니다. 실제 운영 학습은 GRID=8 (총 64 cell) 입니다 (1행 6열).
...
demo 에서는 chip 을 4×4 격자 (총 16 cell) 로 나눈 뒤 무작위로 두 그룹 (각 8 cell) 으로 쪼개 그렸습니다. A label 쪽과 B label 쪽은 **서로 다른 분할** 을 한 번씩 따로 뽑아 사용합니다.

- **FCM mixed (A label)**: B 위에 A 의 group_0 cell 들을 덮어쓴 chip. label 은 A 와 B 두 결함이 모두 들어있다고 부여 → 모델이 multi-label 신호를 학습.
- **FCM mixed (B label)**: 다른 partition 을 한 번 더 뽑아, A 위에 B 의 group_0 cell 들을 덮어쓴 chip. label 동일하게 A∪B → 두 결함 공존 신호를 양쪽 시점에서 함께 학습.
- **Pair Mask (A-only)**: 위 A label 의 mixed chip 에서 group_1 cell 자리만 흰색으로 가린 chip. 가려도 "이건 A 결함이다" 만 학습시켜 A 단독 패턴을 잡고, 흰 배경이 새 결함으로 오인되지 않도록 false-positive 를 같이 억제.
- **Pair Mask (B-only)**: 같은 방식, 단 B label partition 의 group_1 cell 자리를 흰색으로 가린 chip → "이건 B 결함" 만 학습.

A-only 와 B-only 의 흰색 가림 위치가 서로 다른 이유는 partition 을 독립적으로 두 번 뽑기 때문이며, 학습 단계에서 batch 내 다른 pair 들과 paired forward 로 한꺼번에 흘려 background false-positive 까지 같이 억제하는 흐름입니다.
