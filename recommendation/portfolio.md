...
학습 코드 (`_train_chip_variant.py::grid_complete` + `do_pair2`) 의 GRID=8 / 50%-cell 덮어쓰기 + black-fill paired mask 를 실제 chip 이미지에 적용한 결과 (2행 3열, group2 = A + B):

| chip A: scratch | FCM mixed (A base + B cells) | Pair Mask (A-only) |
|:---------------:|:----------------------------:|:------------------:|
| <img src="./figures/fcm_pm_step_a.png" width="160" /> | <img src="./figures/fcm_pm_step_mixed_a.png" width="160" /> | <img src="./figures/fcm_pm_step_masked_a.png" width="160" /> |
| **chip B: scratch_rot** | **FCM mixed (B base + A cells)** | **Pair Mask (B-only)** |
| <img src="./figures/fcm_pm_step_b.png" width="160" /> | <img src="./figures/fcm_pm_step_mixed_b.png" width="160" /> | <img src="./figures/fcm_pm_step_masked_b.png" width="160" /> |

같은 chosen-cell 집합 위에서 A 와 B 양쪽 base 의 FCM mixed 와 Pair Mask 가 같이 만들어집니다. mixed_A 와 mixed_B 는 label 이 모두 A∪B (두 결함 union) 이고, masked_A 는 A label 만 / masked_B 는 B label 만 부여해 paired forward 로 학습시킵니다.
