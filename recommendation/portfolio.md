...
| 2026년 1월 ~ 현재 | P3. Trend Episode 데이터 생성 기반 Anomaly-detection 검증 PoC | 3인 PoC, 본인 70% 리딩. trend sample 10,000개 구성 | 현업에서 BBD / Overlay / CD 담당으로 **9년간** trend 이상 감지를 해 오며 확인한 전형적인 불량 패턴과 정상 chart 산포 / noise 환경을 episode generator parameter 로 코드화했습니다. | 20% | 45% | 35% |
...
| NO | 성명 | Knox Id | 소속 | 역할 | 기여도 |
|----|------|---------|------|------|--------|
| 1 | 최호길 | 개인정보 비공개 | 메모리제조센터 QIE그룹 | chip single / 2-combo 합성, CutMix 계열 선정, Pair Mask loss masking 설계, FCM-PM 본 과제 신규 적용, val_margin 기반 best-model selection, Temperature Scaling, pos/neg target asymmetry sweep, max-prob threshold gate, 4-bag majority voting ensemble, Knowledge Distillation 압축 후보 검토 | 80% |
...
본 과제의 핵심은 chip 하나의 multi-label failure 를 안정적으로 분석·검출하기 위해, 본 과제 데이터 구조에 맞춰 합성 / 손실 / 모델 선택 / 추론 기법을 직접 설계해 결합한 것입니다.
...
- **추론 최적화**: max-prob threshold gate, 4-bag majority voting ensemble 보조 안정성 검토, Knowledge Distillation single student (1× cost) 압축 후보 검토를 대표 성과와 분리해 관리했습니다.
...
추가로 4-bag majority voting ensemble 과 Knowledge Distillation 은 후속 운영성 검토 항목으로 분리했습니다. ensemble 은 seed 와 augmentation 강도 차이에 따른 checkpoint 흔들림을 보는 보조 안정성 실험이고, Knowledge Distillation single student 는 단일 모델 수준의 추론 비용으로 압축 가능성을 보는 후보입니다. 둘 다 bit_F1 이 대표 모델보다 낮은 구간이 있어 제출 성과에는 포함하지 않습니다.

**P2 대표 성과 / 후속 개발 / 한계 및 관리**

- **대표 성과**: FCM-PM val_margin 단일 모델 bit_F1 **0.9943** / Total FAR **0.00%** 입니다.
- **후속 개발**: 4-bag majority voting ensemble 은 실전 운영의 모델 안정성 확보, Knowledge Distillation single student 는 양산 추론 생산성 (single-model latency) 확보를 위한 압축 후보로 검토 중이며, 운영 threshold calibration 과 함께 대표 성과와 분리해 관리합니다.
...
출발은 BCE + Label Smoothing 이었지만 2-combo 신호 자체를 거의 학습하지 못했고, Focal / ASL 같은 loss 변형으로도 false-positive 억제에는 닿지 못했습니다. 다음으로 시도한 CutMix 는 두 가지 문제가 같이 보였는데, 직사각형 영역만 잘라 붙이는 방식이라 정작 결함 위치가 잘려 학습 신호가 사라지는 경우가 있었고, 잘라 붙이지 않은 background 영역까지 failure 로 학습되면서 Normal / Invalid / OOD negative 평가에서 false-positive 가 크게 늘었습니다.
...
- val_margin 으로 고른 epoch 이 val_f1 보다 실제 test bit_F1 을 훨씬 정확히 예측합니다 (Spearman ρ val_margin +0.56 vs val_f1 −0.10). 즉 val_margin 기준 best checkpoint 가 실제 test 성능을 더 안정적으로 끌어올립니다.
...
- **비대칭 target sweep**: positive target 0.95 / negative target 0.30 조합이 pos ∈ {0.75~1.00} × neg ∈ {0~0.30} sweep 에서 negative 측 보수성을 가장 안정적으로 확보했습니다.
- **Majority Voting Ensemble**: 서로 다른 augmentation 강도의 4 bag majority voting 으로 실전 운영 모델 안정성을 확보하는 보조 실험입니다.
...
**[데이터]** 학습 원천은 **[실전 현업 chip failure 원본]** 실제 EDS Failbit Map 불량과 유사하게 본인이 생성한 단일 결함 4 class 입니다. 그 위에 **[FCM-PM 합성, 실전 chip 원천 결합]** 2-combo 6 종을 반도체 도메인 분포에 맞춰 만들어 multi-label 학습 신호를 확보했고, 평가 측은 여기에 **[현업 분포 기반 합성 평가셋]** Normal + Invalid + OOD 4 종을 더해 single 4 + 2-combo 6 + negative 6 = 16+ class × 약 3,850 chip 규모의 controlled benchmark 를 구성했습니다. 합성 detail 은 다음 세 축으로 잡았습니다. (1) **2-combo 6 종**: 단일 결함 4 class 의 모든 가능한 조합 (4C2 = 6) 을 포함. (2) **Normal 합성**: per-chip Beta(2, 10) noise 확률분포 (평균 ~0.17) 로 palette Grade 0~2 만 사용해 실전 정상 chip 분포를 모사. (3) **Negative 평가셋**: Invalid 는 white interior + orange border + black text, OOD wafer-pattern 4 종, OOD synthetic 은 trained 2 종 + untrained 1 종 결합 4 종으로 설계해 신규 failure family 모사까지 포함했습니다. 실전 평가 환경을 얻기 어려운 조건에서 현업 분포에 맞춰 평가셋까지 직접 설계한 부분이 본 과제 데이터 전략의 핵심입니다.
...
│ 실제 EDS Failbit Map 불량과 유사하게 본인이 생성한 단일 결함 4 class      │
...
│ target = failure label union, 2-combo 6종 (4 class C2 전수)               │
│ Negative: Normal Beta(2,10) noise + Invalid + OOD synthetic               │
│ val_margin = mean(pos bit score) − max(neg bit score)                     │
│ Spearman ρ(epoch, test_f1): val_margin +0.56 / val_f1 −0.10               │
│ 단일 checkpoint 안정성: 인접 epoch + 4-bag ensemble 일관성 점검            │
└──────────────────────────────────┬────────────────────────────────────────┘
                                   ▼
┌─ Step 6: Inference 단 운영 안전판 ───────────────────────────────────────┐
│ max-prob threshold gate: max-prob<0.55 → Normal 강제 (운영 80% Normal)    │
│ pos=0.95 / neg=0.30 비대칭 target (39-cell sweep)                          │
│ 4-bag majority voting ensemble: teacher / 상한 확인 보조                   │
│ Knowledge Distillation single student: 1× cost 압축 후보 (collapse 해소)   │
└──────────────────────────────────┬────────────────────────────────────────┘
                                   ▼
┌─ OUTPUT: FCM-PM val_margin best checkpoint ─────────────────────────────┐
│ bit_F1 0.9943 / Total FAR 0.00% / 1x latency / 1x throughput / 1x params  │
│ ablation: Pair Mask 제거 시 Total FAR 100% → background loss 핵심 근거    │
└───────────────────────────────────────────────────────────────────────────┘
```
...
- **CutMix 계열 채택**: 영역 단위로 원값을 보존해 붙이는 CutMix 계열이 양자화 의미 보존 측면에서 적합하다고 본인이 판단해 채택했고, 이후 박은병 교수 자문에서 본 과제 데이터 특성에 맞는 선택임을 확인했습니다.
- **Full-Cover Mixup 확장**: 일반 CutMix 는 일부 직사각형 영역만 잘라 붙입니다. chip 내부 어디에 failure 가 올지 모르는 본 과제에서는 failure signal 이 잘릴 위험이 남아, chip 전체 grid 를 cover 하는 Full-Cover Mixup 으로 확장했습니다.
...
운영 환경 약 80% Normal 분포에 대응해, max-prob<0.55 입력을 Normal 로 강제하는 threshold gate 를 두어 운영 false-positive 를 한 단계 더 억제했습니다. pos=0.95 / neg=0.30 비대칭 target sweep 으로 negative 측 보수성을 강화했고, 4-bag majority voting ensemble 은 bag 별로 서로 다른 augmentation 강도를 적용해 오분류 패턴이 겹치지 않게 분산을 줄였습니다. Knowledge Distillation (single student) 는 CutMix 활성 batch 의 distillation loss 를 제외하는 설정으로 학생 모델 collapse 를 회피, bit_F1 **0.9265** / Total FAR **0.00%** 까지 동작했지만 대표 모델보다 낮아 후속 압축 후보로만 두었습니다.
...
- **pos / neg target sweep**: pos=0.95 / neg=0.30 비대칭 target 조합이 sweep 에서 bit_F1 0.9795 / Total FAR 0.00% 를 가장 안정적으로 동시에 달성했습니다. neg target 을 낮추면 OOD FAR 가 올라가고, neg 만 높이면 bit_F1 자체가 빠졌습니다.
...
- **기술 지표** (단계별 적용 효과):
  ...
  - 운영 안전판: max-prob threshold gate + 4-bag majority voting ensemble + Knowledge Distillation single student 압축 후보.

  **Multi-label 학습 recipe 성능표 [현업 failure chip 원천 + 도메인 확률분포 기반 생성/검증]**

  | # | Recipe | bit_F1 | single | 2combo | FAR | NI-FAR | OOD-FAR | Latency | Throughput | Params | Note |
  |---|--------|--------|--------|--------|-----|--------|---------|---------|------------|--------|------|
  | 1 | BCE + Label Smoothing | 0.1093 | TBD | TBD | 99.47% | TBD | TBD | 1x | 1x | 1x | ladder baseline |
  ...
  | 4 | CutMix (random rectangle) | 0.9359 | TBD | TBD | 42.05% | TBD | TBD | 1x | 1x | 1x | ladder baseline |
  ...
  | 7 | **FCM-PM + val_margin selection** | **0.9943** | - | - | **0.00%** | 0.00% | 0.00% | 1x | 1x | 1x | **제출 대표 모델** |
  | 8 | 4-bag Majority Voting Ensemble※ | 0.9615 | 1.0000 | 0.9475 | 0.00% | 0.00% | 0.00% | 4x | 1/4x | 4x | teacher / 상한 확인 |
  | 9 | Knowledge Distillation (single student) | TBD | TBD | TBD | TBD | TBD | TBD | 1x | 1x | 1x | 압축 후보 |

  ※ row 8 의 ensemble 은 보조 안정성 실험으로, 01_career_profile.md / 02_ai_portfolio.md 의 ensemble 수치와는 동일 실험이 아닙니다. 표 row 1~7 흐름을 보면 CutMix 단독으로는 FAR 이 운영에 쓰기 어려운 구간에 남고, FCM-PM + val_margin 으로 와서야 bit_F1 0.9943 / Total FAR 0.00% 의 균형이 맞춰집니다. 제출 대표 성과는 row 7 (bit_F1 0.9943 / Total FAR 0.00%) 로 고정합니다.
...
│ Rendering: 224×224 PNG, train 7K + val 1.5K + test 1.5K = 10,000 trend sample│
└──────────────────────────────────┬────────────────────────────────────────┘
                                   │
                                   │
                                   ▼
...
- **기술 지표** (단계별 적용 효과):
  - 데이터: trend sample 총 **10,000개** (train 7K + val 1.5K + test 1.5K), test 평가셋 normal 750 + abnormal 5종 각 150 = **1,500개**.
...
**P3 결론**: 본인 trend 판정 경험을 Region / Noise / anomaly parameter 로 코드화한 **10,000개 합성 trend sample** 데이터 자산 + Binary F1 **0.9967** 학습·검증 완료 (현업 데이터 적용 직전). 실전 abnormal log 기반 generator 재보정 trigger 와 type 세분화 분류기는 후속 단계.
