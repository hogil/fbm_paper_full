...
학습 코드 (`_train_chip_variant.py::grid_complete`) 의 GRID=8 / 50%-cell 덮어쓰기 + corner-fill paired mask 를 실제 chip 이미지에 적용한 결과:
...
좌측부터 chip A (bank_boundary), chip B (fork), FCM (A 위에 B 의 8×8 격자 절반을 덮어쓴 mixed chip), PM (FCM 에서 B 가 들어간 자리만 A 의 corner 평균 색으로 mask 한 paired augmentation chip) 입니다.
