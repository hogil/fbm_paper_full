...
데이터 측면은 wafer 당 약 1,000만 cell 의 hex → Grade 변환이 Python loop 로 처리 시간이 오래 걸리고 데이터 용량도 커서 적재 / 조회 비용 압박이 있었습니다. Known 측면은 label 이 16 class / 1,500 장 으로 supervised 학습 데이터가 부족했고, 16 class 중 center 영역처럼 일부 class 들의 불량 위치가 겹치는 경우 오분류하는 문제가 발생했습니다. Unknown 측면은 운영 환경에 다수의 unknown failure group 과 noise group 이 존재해 supervised 정량 metric 이 현 상황에는 맞지 않았습니다. 운영 측면에서는 일 약 2만 장 wafer 의 1시간 주기 적재와 운영 뷰어 응답성을 만족시키면서 모델 / 적재 / 조회 비용까지 낮춰야 했습니다.
...
좌측 raw wafer 는 Stage 1 wafer-level CNN 은 비슷한 불량으로 오인되는 사례입니다. Stage 2 ROI YOLO 는 ROI 영역에서 chip 단위 object detection 으로 각 chip 의 불량 class 를 분류한 뒤, 그 chip class 분포로 wafer 의 최종 class 를 예측하는 구조입니다.
...
- **기술 지표**
  ...
  **Unknown contrastive 구성요소 성능표 [생성 데이터로 개발]**
  ...
  | 6 | 최종 recipe (Local DenseCL 제외 4-tool) | 0.9559 | 6.66% | 0.9660 | 0.8208 | 0.5648 | 0.8861 | 0.781 |
  | 7 | 최종 recipe + 후처리 (noise 임계 τ=0.5) | 0.9619 | 0.00% | 0.9679 | 0.8184 | 0.5489 | 0.8856 | 0.781 |


- **현업 임팩트**: 설비 불량 조기 감지 및 불량 wafer list 확보가 가능해집니다. 운영 뷰어는 2025년 5월부터 DRAM 전제품 라인 양산 운영 중이며 **DRAM D1a/b/c/d 분석 파트에서 매일 사용 중**, **공수 약 90% 절감 (연 약 26억 효과)** 과 **수율 +0.02% 개선 (P3WN 1건 약 97억 효과)** 의 비즈니스 효과를 창출했습니다. AI 모델을 구동하기 위한 GPU 도 수요 조사를 통해 확보했습니다 (GPU 할당 2026년 9월).
...
환경 측면은 중복 failure label 확보가 어려운데 실제로 중복 failure 가 발생하고, Normal / Invalid / OOD 까지 있어 noise 도 많은 상황이었습니다. 기술적 측면은 최대한 현업과 유사한 환경을 구성하기 위해 train 은 single 만으로 진행해야 해서 eval prediction 성능 향상이 어려운 조건이었습니다.
...
현업 EDS Failbit Map 에서 발생할 만한 single failure pattern 4 종 (bank_boundary / fork / scratch / scratch_rot) 을 도메인 경험으로 정의하고, 공간 분포와 grade 강도를 최대한 현업과 비슷하게 재현하기 위해 class 별 alpha mask 함수와 intensity tier 별 grade 0-7 categorical sampling 분포를 통해 생성했습니다. train 은 single 4 class 만 두고, eval 은 single 4 종 + 2-combo 6 종에 운영에서 함께 발생하는 Normal / Invalid / OOD chip 까지 모두 생성해 포함하여, 약 3,850 chip 의 평가셋을 구성했습니다.
...
```
# Per-class probability (K classes)
p_k = sigmoid(z_k),   k = 1, ..., K

# BCE loss (1 vs other class sigmoid, not softmax CE)
L_BCE = -Σ_k [ y_k log p_k + (1-y_k) log(1-p_k) ]

# Full-Cover CutMix  (A, B = two single-defect chips)
x_mixed  = paste B grid cells onto A
y_mixed  = y_A ∪ y_B

# Pair Mask  (A-only paired augmentation)
x_masked = mask B-pasted cells of A
y_masked = y_A

# Total loss
L_total = L_BCE(x_mixed, y_mixed) + w · L_BCE(x_masked, y_masked)

# Best-model selection  (positive bit prob - negative bit prob)
val_margin = mean(p_positive) - max(p_negative)
...
val_margin 은 positive bit 평균 score 와 negative bit 최대 score 의 차이로 정의했습니다. val_f1 보다 val_margin 이 test bit_F1 과의 상관성이 더 높았습니다.
...
augmentation 은 CutMix → CutMix + Pair → FCM-PM 순서로 직접 비교했고, false-positive 와 bit_F1 의 trade-off 가 어느 조합에서 열화되는지는 **[구현 성과]** ablation 표를 보면 됩니다.

- **cover grid sweep**: CutMix 합성 시 chip 을 가로 세로 몇 조각으로 나눌지 (multiplier) 와 그 조각들을 full cover 시키기 위해 몇 group 으로 묶을지 (partition) 를 다양하게 비교했고, **partition=3 / multiplier=1 조합에서 bit_F1 0.9960** 으로 가장 좋았습니다.
- **분할 선택 근거**: 더 잘게 나눌수록 결함 영역이 작은 조각으로 쪼개져 모델이 결함 형태를 인식하기 어려워졌고, 4 분할 이상에서는 분류 정확도가 오히려 떨어졌습니다. 3 분할 부근이 데이터에 적합했습니다.
...
**P3 핵심 요약**: 실전 abnormal label 부족으로 trend anomaly 모델 검증이 막혀 있던 한계를 풀기 위해, 본인 BBD / Overlay / CD 담당 **10년** trend 판정 경험을 generator parameter (Region 5종 / Noise 3종 / Anomaly 5종) 로 옮겨 trend sample **약 7,000개** (normal 3,500 + abnormal 3,500) 를 만들고, 1차 Binary gate baseline 으로 **Binary F1 0.9967** 까지 확인한 PoC 입니다. 현재 실제 현업 데이터 적용 직전 단계입니다.
...
trend chart 판정 경험을 generator parameter 로 옮겨 trend sample 약 7,000개 (normal 3,500 + abnormal 3,500) 를 만들었습니다. 계측 밀도 Region 5단계 (dense / sparse / very_sparse / thin / missing), Noise 3분포 (Gaussian / Laplacian / Correlated), Anomaly 5종 (mean shift / standard deviation / spike / drift / context) 을 generator parameter 에 반영했습니다.
...
trend sample 을 normal / abnormal 두 class 로 학습하는 1차 Binary gate baseline 분류기로 시작했습니다. P3 는 데이터 생성이 주 성과인 PoC 라 모델은 단순 baseline 으로 두고, 전체 단계는 아래 logic flow 와 같습니다.
...
+---------------------------------------------------------------------------+
                                    ...
                                    v
+---------------------------------------------------------------------------+
|  [BUILD DATASET]  encode domain distribution + bounded synthetic-normal   |
...
+---------------------------------------------------------------------------+
                                    ...
                                    v
+---------------------------------------------------------------------------+
|  [TRAIN]  ConvNeXtV2-Tiny + Focal Loss + EMA + weighted sampler           |
|  -> 5-seed multi-run with val-F1 median smoothing for robustness          |
+---------------------------------------------------------------------------+
                                    |
                                    v
+---------------------------------------------------------------------------+
...
- **class-balanced weighted sampler**: 학습 batch 의 normal / abnormal prior 가 한쪽으로 치우치지 않도록 weighted sampler 로 보정해 학습 안정성을 확보했습니다.
...
- **5-seed multi-run robustness**: seed 변화에 따른 학습 안정성을 5-seed 학습으로 확인했고, best F1 0.9987 / 평균 F1 0.9967 으로 seed 둔감성을 확보했습니다.
- **재현성 seed 관리**: DataLoader worker seed / 분산 학습 seed 까지 고정해 학습 결과 재현성을 유지했습니다.
