...
학습 코드 (`_train_chip_variant.py::grid_complete` + `do_pair2`) 의 GRID=8 / 50%-cell 덮어쓰기 + black-fill paired mask 를 실제 chip 이미지에 적용한 결과 (1행 6열, group2 = A + B):

| chip A: scratch | chip B: scratch_rot | FCM mixed (A base) | FCM mixed (B base) | Pair Mask (A-only) | Pair Mask (B-only) |
|:---------------:|:-------------------:|:------------------:|:------------------:|:------------------:|:------------------:|
...
chip 을 8×8 격자 (총 64 cell) 로 나눈 뒤, 그 중 절반 (32 cell) 을 무작위로 고릅니다. 이 한 번 고른 32 cell 위치를 A 와 B 양쪽에 똑같이 쓰면서 4 장의 학습 chip 을 함께 만듭니다.

- **FCM mixed (A base)**: A 의 고른 32 cell 자리를 B 의 같은 자리 픽셀로 덮어씀 → label 은 A 와 B 두 결함을 모두 가지고 있다고 부여.
- **FCM mixed (B base)**: 반대로 B 의 같은 32 cell 자리를 A 픽셀로 덮어씀 → 위와 동일하게 두 결함 모두로 label 부여.
- **Pair Mask (A-only)**: A 의 같은 32 cell 자리만 검정으로 가린 chip → 가려진 영역이 있어도 "이건 여전히 A 다" 라고 A 단독 label 로만 학습.
- **Pair Mask (B-only)**: 같은 자리 32 cell 을 B 에서 검정으로 가린 chip → "이건 여전히 B 다" 라고 B 단독 label 로 학습.

mixed 두 장은 모델에게 두 결함이 같은 chip 에 공존하는 multi-label 신호를 주고, masked 두 장은 일부 영역이 가려져도 단일 결함을 끝까지 인식하도록 single class 패턴을 다시 잡아 줍니다. 네 장을 같은 학습 step 에서 paired forward 로 한꺼번에 흘려 background 의 false-positive 까지 같이 억제합니다.
