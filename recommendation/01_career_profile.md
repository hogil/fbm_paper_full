...
Failbit Map 을 대량 변환, 조회하고 Known/Unknown 불량 후보를 현업 검토로 연결한 양산 운영 과제입니다. wafer 전체 분포, chip 내부 판단 근거, 발생 위치 정보를 나눠 보도록 모델 구조를 잡았습니다.
...
multi-label 조합 label 을 충분히 모으기 어려운 실제 조건을 가정하고, single-label 불량 chip 을 조합해 multi-label 불량을 예측하도록 만든 PoC 입니다. 핵심은 모델보다 학습 데이터 조합과 배경 영역 loss 처리였습니다.

(1) 과제 개요 및 규모 / 담당 역할 / 수행 업무 / 성과

- 과제 개요 및 규모: single 4 class 기반으로 2-combo, Normal, Invalid, OOD 를 포함한 16+ class × 약 3,850 chip 통제 합성 평가셋을 구성했습니다.
- 담당 역할: 본인 80% / 관리자 20%
