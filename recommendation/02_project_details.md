...
## 3. P3. Trend Episode 데이터 생성 기반 Anomaly Detection
...
| 수행기간 | 2025년 ~ 현재 (1년+) |
| 운영 상태 | **[합성 trend chart PoC]** - normal 750 + abnormal 5종 각 150 = 총 1,500 sample trend episode 합성 데이터셋 생성 완료. Binary gate + Type classifier 는 생성 데이터가 정상/이상 패턴을 학습 가능하게 담고 있는지 확인한 검증용 baseline 입니다. |
| 장기 확장 방향 | trend 합성 데이터 생성 체계를 semi image / EDS test data / engineer history / 공정 config 로 확장 |

### 3.2 참여 인력 및 역할

| 인력 | 역할 | 기여도 |
|------|------|--------|
| **본인** | episode 합성, noise 3종 매핑, 일반 trend 불량 4종 + context 1종 합성, 생성 데이터 학습 가능성 검증, 전체 설계 및 구현 직접 수행 | **90%** |
| 관리자 | 매니징 (방향성 / 일정 / 리뷰) | **10%** |

### 3.3 개인별 기여 서술

P3의 main 성과는 **모델이 아니라 데이터 생성**입니다. 본인은 CD overlay trend, noise 분포, baseline 평탄도, spec-in 변동 사고 가능성에 대한 현업 인지를 episode 합성 generator 로 코드화했고, Binary gate + Type classifier 는 이 생성 데이터가 정상/이상 패턴을 학습 가능하게 담고 있는지 확인하기 위한 검증용 baseline 으로 사용했습니다.
...
- **생성 데이터 검증용 baseline 학습 안정화**: 생성 데이터 학습 가능성을 확인하는 과정에서 일시적 지표 spike 에 의해 checkpoint 선택이 흔들리지 않도록 최근 window 의 val-F1 median 과 val-loss spike guard 를 함께 사용
- **검증용 baseline** (Binary gate + Type classifier) 으로 생성 데이터의 학습 가능성 확인
- 생성 데이터 기반 학습 수렴 확인 및 튜닝 직접 수행
...
**데이터 수집 및 전처리 경로**: P3는 실전 trend 데이터를 그대로 수집해 학습한 과제가 아니라, 양산 fleet trend chart (현업 측정 및 관리 chart) 의 통계 특성 (정상 fleet_mean / fleet_within_std / fleet_between_std) 을 합성 generator 의 파라미터로 직접 코드화한 과제입니다. 학습 입력은 224×224 chart PNG (baseline_line + fleet overlay 렌더링) 로 통일하며, 평가는 normal 750 + abnormal 5종 각 150 = 총 1,500 sample 합성 trend chart 평가셋으로 확인합니다.
...
**(4) 생성 데이터 검증용 baseline 학습 안정화**

생성 데이터가 정상/이상 패턴을 학습 가능하게 담고 있는지 확인하려면 검증용 baseline 의 학습 수렴이 안정적으로 보여야 합니다. 학습 중 일시적 지표 spike 에 의해 checkpoint 선택이 흔들리지 않도록 최근 window 의 val-F1 대표값을 평균이 아니라 median 으로 잡고, 동시에 val-loss spike guard 를 적용했습니다.
...
[일시 spike 영향 완화]
          │
          ▼
[생성 데이터 검증용 baseline 학습 안정화]
```

위 안정화 결과 5-seed sweep 의 Binary F1 분포가 0.9944~0.9988 범위로 좁아져, 생성 데이터 기반 검증용 baseline 이 안정적으로 수렴함을 확인했습니다.
...
└────────────────────┬────────────────────┘
            ...
            [PNG 렌더링 → trend chart]
              ...
              [검증용 baseline 학습]
                     │
                     ▼
   [PoC 학습 수렴 확인 - 성능 수치는 생성 데이터 품질 확인용 참고 수치]
```

### 3.6 구현 성과

이 과제의 핵심 성과는 **현업 도메인 자산을 합성 generator 의 모든 파라미터로 직접 코드화하여 trend episode 데이터를 생성한 점**입니다. Binary F1 / Recall 은 AI 모델 차별성 주장이 아니라, 생성 데이터가 정상/이상 패턴을 학습 가능하게 담고 있는지 확인한 참고 수치입니다. **실전 양산 데이터 검증은 미실시** 상태입니다.
...
본 과제의 본질은 먼저 **trend episode 합성 데이터 생성**입니다. trend / semi image / EDS test data / engineer history / 공정 config 연계는 이 데이터 생성 체계를 확장하는 후속 방향입니다.

| 개발 modality | 상태 | 내용 |
|--------------|------|------|
| trend chart 단일 | **[합성 PoC 완료]** | Region 5종 + Noise 3종 + 일반 trend 불량 4종 + context 1종 합성 generator 직접 설계, Binary F1 0.9967 은 생성 데이터 학습 가능성 확인 참고 수치 |
...
| **multi-modal Orchestration Agent** | **[후속 확장 방향]** | trend 합성 데이터 생성 체계를 다른 입력 축으로 확장 |
...
위 5 항목은 PoC 평가 metric 보다 이 과제의 합격 결정 요소이며, **반도체 역량을 AI 학습 데이터 생성 체계에 직접 반영해 현업 문제를 개선** 하는 흐름에서 "도메인 역량" 의 직접 코드화 증거입니다. P3는 특별한 anomaly 모델을 만든 과제가 아니라, 현업 trend 이상 양상을 AI 학습 데이터로 만들 수 있게 한 **도메인 기반 데이터 생성 과제**로 평가받습니다.
