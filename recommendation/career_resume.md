...
- **본 과제 chip Grade 이미지 조건**: 결함 픽셀 의미 보존, failure signal coverage 보장, background 오학습 억제가 동시 필요.
...
- 수행 업무:
  ...
  - **추론**: max-prob threshold gate (max-prob<0.55 → Normal 강제), pos=0.95 / neg=0.30 비대칭 target, Temperature Scaling, 4-bag majority voting ensemble 검증.
  - **Knowledge Distillation**: single student 가 CutMix 활성 batch 의 distillation loss 를 제외하는 설정으로 학생 모델 collapse 를 회피, bit_F1 0.9265 / Total FAR 0.00% 까지 측정해 후속 압축 후보성을 확인했습니다.
...
- 성과: **[실전 현업 chip failure 원본]** single 4 class 학습 원천 위에 **[FCM-PM 합성, 실전 chip 원천 결합]** 2-combo 6 종을 도메인 분포로 합성해 학습 / 평가 가능화. FCM-PM 대표 모델 bit F1 **0.9943**, **[현업 분포 기반 합성 평가셋]** Normal / Invalid / OOD negative 에서 false-positive 미검출 (Total FAR 0.00%).
...
- 추가로 Knowledge Distillation single student 는 1× inference cost 압축 후보로 검토했고, bit_F1 **0.9265** / Total FAR **0.00%** 로 collapse 없이 동작함을 확인했습니다. 다만 단일 FCM-PM val_margin 대표 모델보다 bit_F1 이 낮아 제출 성과가 아니라 후속 압축 후보로 분리합니다. 4-bag majority voting ensemble 은 seed / checkpoint 안정성을 확인하기 위한 보조 실험으로만 두고, 제출 대표 성과는 FCM-PM val_margin 단일 모델 bit F1 **0.9943** / Normal / Invalid / OOD negative Total FAR **0.00%** 로 고정했습니다. P2 는 실전 single failure chip 원천 위에 부족한 2-combo 결함을 반도체 도메인 (Grade 0-7 의미, failure 위치 / 강도 / 조합 분포) 으로 합성하고 multi-label 학습 / 평가 설계까지 결합한 방법론입니다.

**P2 대표 성과 / 후속 개발 / 한계 및 관리**

- **대표 성과**: FCM-PM val_margin 단일 모델 bit_F1 0.9943 / Total FAR 0.00% 입니다.
- **후속 개발**: Knowledge Distillation single student (bit_F1 0.9265 / Total FAR 0.00%), 4-bag majority voting ensemble 안정성 비교, 운영 threshold calibration 은 대표 성과와 분리해 관리합니다.
- **한계 및 관리**: P2 결과는 현업 single failure chip 원천에서 출발해 도메인 확률분포 기반으로 구성한 controlled benchmark 기준입니다. 본 P2 의 failure class 는 실제 EDS Failbit Map 에서 관찰되는 failure 형태를 기준으로 구성했습니다. EDS Failbit Map 은 Test 결과가 0~7 Grade 로 표현된 map 이며, **Grade 0 은 양호, Grade 7 은 가장 강한 failure 상태**입니다. 본 P2 benchmark 는 이번 생성 데이터 기준으로 정의한 **single failure 4 class** 에서 가능한 **2-combo 6 종 (4C2)** 을 모두 포함하도록 구성했고, 합성 측은 pixel-level MIN-BLEND 로 Grade 원값을 보존하면서 Full-Cover Mixup 으로 chip 전체 grid 를 cover 했고 Normal 은 per-chip Beta(2, 10) noise 확률분포로 실전 정상 chip 분포를 모사했습니다. 실제 운영에서 추가될 수 있는 신규 failure class 와 장기 운영 분포 변화는 후속 확장 및 검증 대상으로 분리합니다. label 부족 조건에서 2-combo 학습 신호와 false alarm 억제 구조를 검증한 결과로 해석합니다.
...
- **핵심 제어**: Pair Mask background loss masking, val_margin 기반 checkpoint selection, Label Smoothing, Temperature Scaling, max-prob threshold gate, 4-bag majority voting ensemble 보조 안정성 검토를 함께 구성했습니다.
...
- **Full-Cover Mixup 확장**: 일반 CutMix 는 일부 직사각형 영역만 잘라 붙입니다. chip 내부 failure 위치를 사전에 알 수 없는 본 과제에서는 failure signal 이 잘릴 위험이 남아, chip 전체 grid 를 cover 하는 Full-Cover Mixup 으로 확장했습니다.
...
- 성과: **[합성 trend chart, PoC]** train 7,000 + val 1,500 + test 1,500 = 총 **10,000개 trend sample** 을 구성했고, PoC metric 은 test 평가셋 normal 750 + abnormal 5종 각 150 = 총 **1,500개** 기준으로 보고합니다.
- 추가로 1차 Binary gate baseline 은 test split 1,500건 / normal threshold=0.9 기준 Binary F1 **0.9967**, Abnormal Recall **0.9987** (TN/FN/FP/TP=746/1/4/749) 로, 생성 데이터가 정상 / 이상 패턴을 학습 가능한 형태로 담고 있음이 확인되었습니다. 판정식은 `p_anom = sigmoid(f_theta(x))`, `y_hat = 1 if p_anom >= 0.9 else 0` 입니다.

**P3 대표 성과 / 후속 개발 / 한계 및 관리**

- **대표 성과**: 본인 trend 판정 경험을 Region / Noise / anomaly parameter 로 옮겨 총 **10,000개** 합성 trend sample 데이터 자산을 만들었습니다.
