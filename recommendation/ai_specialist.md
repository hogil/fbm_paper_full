...
지원자는 13년간 메모리 QIE에서 축적한 현장 원리(웨이퍼 분포, 결함 발생 모드, 설비 노이즈 특성, L1 trend의 물리적 의미, fleet 항상성)를 AI 모델·데이터·평가의 각 의사결정에 그대로 반영한다. 단순히 SOTA 백본을 가져다 쓰는 것이 아니라, "이 도메인에서는 flip 증강이 왜 틀린지, BICUBIC 보간이 왜 의미를 깨뜨리는지, 작은 val set의 saturation이 왜 진짜 성능 신호를 가리는지, fleet의 SNR이 왜 enforcement floor를 결정해야 하는지"를 구현 단계까지 일관되게 반영하여 설계한다. 이 융합이 본 추천서의 가장 결정적인 차별 인자다.
...
| 파이프라인 / 데이터 가속 | ✓ | Cython 100배, Palette PNG 75% 절감 | I/O 병목 제거 |
...
| 수상 | 주관 | 내용 |
|------|------|------|
| **AI BP Festival — Challenge 수상** [202X년] | DS부문 AI센터 | DS부문 단위 AI Best Practice Festival 경연에서 Challenge 부문 수상 |
| **MEMORY PEER AWARD** 수상 [202X년] | 메모리 사업부 | 동료 추천 기반 사업부 단위 우수 인력 표창 |
| **MTC 고등급제안 1등급** [202X년] | Memory Technology Conference | 사내 기술 제안 경연에서 고등급 1등급 평가 |
| **메모리 우수강사 수상 (DX 강의)** [202X년] | 메모리 사업부 | 사내 DX/AI 관련 강의 우수 강사 인정 |

> 위 4건 모두 AI 또는 DX 관련 성과이며, 단순 단발 프로젝트 성과가 아니라 사내 다양한 평가 채널(센터 주관 경연 / 동료 평가 / 기술 제안 평가 / 강의 평가)에서 누적적으로 인정받은 이력이다. 사내 AI Specialist 후보로서의 다각적 검증이 이미 완료된 상태로 볼 수 있다.
...
|---|--------|------|------------|----------|----------|---------------------------|
| P1 | FBM 합성 + Known/Unknown 2-Track | WM-811K 분포 학습 + FBM 합성으로 known 33-class 자동 분류 및 unknown discovery 동시 수행 | 합성 wafer ~20,000장/일, eval 9,600 PNG | 핵심 설계자 + 직접 코딩 (85-90%) | Known F1 0.9879 / Unknown ARI 0.859, P1 capture 100% | block_expand(BICUBIC→categorical), Cython 100배, Palette 75% 절감, NO-flip 증강 |
...
![Fig 1: 전체 시스템 개요](D:/project/fbm_paper/recommendation/figures/hybrid_fig1_overview.png)
...
**본인의 차별적 기여:** 본 과제의 주요 설계 결정 — (a) WM-811K로부터 cell-level P(defect) 분포를 학습하는 방식 선택, (b) Lorentzian sharp+heavy-tail alpha 18-20회 시행착오, (c) BICUBIC이 categorical 신호를 깨뜨린다는 도달, (d) FCMAE TAPT를 frozen으로 둘 것인지의 판단, (e) Queue+NeCo+NEG 조합과 Local 제거의 ablation 등 — 은 모두 본인의 도메인 직관과 AI 통찰에서 출발했다.
...
| 정확도 (도메인 추정) | mAP@50 약 0.6-0.7 (chip 크기가 작아 detection 한계) | chip CNN val_f1 1.000 / test_f1 0.9799 (실측) |
...
![Fig 2: 분포 학습](D:/project/fbm_paper/recommendation/figures/hybrid_fig2_distribution.png)
...
**RGB 방식이면 (4)와 (5)는 사실상 불가능하다.** RGB는 모든 이미지의 모든 픽셀을 실시간 검색·변경해야 하므로 web app에서 "현업 엔지니어가 각자 다른 결함을 다른 색상으로 분석"하는 시나리오는 비현실적이다. Palette는 PLTE 청크(256색 색상표)만 교체하면 픽셀 데이터는 그대로 둔 채 ms 내에 개인 색상이 즉시 적용된다. **Palette 선택은 일반 CV 엔지니어가 도달하기 어려운 도메인-기술 융합 결정이다 (RGB가 CV 표준이고, Palette는 도메인 의미를 알아야 선택 가능).**
...
![Fig 3: 객체 매칭](D:/project/fbm_paper/recommendation/figures/hybrid_fig3_matching.png)

*Fig 3. Object 매칭 — chip-level 예측이 wafer 좌표계의 obj_id_map으로 어떻게 합성되는지의 시각화.*

![Fig 4: 플로우차트](D:/project/fbm_paper/recommendation/figures/hybrid_fig4_flowchart.png)
...
![Fig 5: 확대 예시](D:/project/fbm_paper/recommendation/figures/hybrid_fig5_zoomed_example.png)

*Fig 5. 합성 wafer 확대 예시 — chip 단위 obj_id, palette 인코딩, fail-bit 분포가 어떻게 한 캔버스에 표현되는지.*

![Fig 6: 혼동 페어](D:/project/fbm_paper/recommendation/figures/hybrid_fig6_confusion_pairs.png)
...
> **DRAM YE팀과 Flash YE팀 모두 본 과제(군)를 핵심과제로 등록.** 인프라(web app, EDS 이미지 수집·변환 파이프라인)는 양 사업부 전수 운영 중이며, AI 모델 부분(Known 2-stage, Unknown contrastive)은 양 사업부 데이터셋 검증을 완료하고 전수 적용 단계적 확대가 예정되어 있다.
...
> 본 모델은 DRAM YE / Flash YE 양 사업부의 핵심과제군에 포함되어 PoC 단계에서 데이터셋 검증을 완료. Chip-level multi-label 은 wafer-level 분류 (P1) 의 입력 신호로도 활용되며 P1 시스템과 연계된다.
...
**본인의 차별적 기여:** 본 프로젝트는 "AI 모델은 충분히 강하다, 문제는 데이터를 얼마나 도메인 충실하게 생성하느냐"가 핵심이다. 본인이 13년간 본 fleet의 실제 노이즈 통계·고장 모드를 직접 episode 구조와 noise type 매핑에 반영. 사내 일반 AI 엔지니어 단독으로는 단순 Gaussian noise만 주입하지만, 본인은 **Gaussian(상태변동) / Laplacian(spike 전조) / AR(1)(drift)의 3종을 실제 설비 모드와 1:1 매칭**하였다.
...
**산식 명시:** 5-seed F1 평균 = (0.9967 + 0.9953 + 0.9980 + 0.9987 + 0.9980) / 5 = **0.9973**.

> **Abnormal recall seed 별 raw 수치는 본 추천서 작성 시점 로그 재확인이 필요하여 [TBD — 로그 재확인 시 보강]. 5-seed 평균만 보고 가능하며 평균값은 약 0.997 수준.**
...
> **DRAM YE팀과 Flash YE팀 모두 본 과제(군)를 핵심과제로 등록.** L1 trend 자동화는 양 사업부의 일상 운영에 가장 직접적으로 시간을 돌려주는 솔루션이다.
...
위 모든 정량 효과는 **Palette PNG + Cython + dual-bucket 의 세 단일 결정이 누적된 결과**이다. 각각은 도메인 의미(grade 0-7 + BIN + 배경 등 32색만으로 충분, hex-grade 매핑이 산술이 아니라 lookup, primary fail map ↔ measure log 시간차의 물리적 의미)를 이해하지 않으면 도달하기 어려운 결정이며, 그 결과 단순 CV 표준 (RGB + pure Python + 단일 source) 대비 수십~수백 배의 운영 효율 차이를 만들어낸다.
...
2. **AI 기술 깊이 (직접 설계·구현).** FCM-PM 신기법, val-margin 선택, Queue+NeCo+NEG contrastive recipe, block_expand_2d, Cython 100배 가속, Palette PNG 75% 절감 등 신기법·최적화를 직접 제안하고 코드로 구현.
...
5. **사업부 간 핵심과제 등록 + 누적 수상 실적.** DRAM YE / Flash YE 양 사업부에서 본 과제군을 핵심과제로 등록. AI BP Festival Challenge, MEMORY PEER AWARD, MTC 고등급제안 1등급, 메모리 우수강사 4건의 누적 수상 — 단일 사업부 단발 성과가 아니라 다채널 검증.
...
| 수상 | AI BP Festival Challenge | DS부문 AI센터 주관 [202X년] |
| 수상 | MEMORY PEER AWARD | 메모리 사업부 동료 추천 [202X년] |
| 수상 | MTC 고등급제안 1등급 | Memory Technology Conference [202X년] |
| 수상 | 메모리 우수강사 (DX 강의) | 메모리 사업부 [202X년] |
| 사업부 채택 | DRAM YE | **핵심과제 등록** (인프라 전수 운영 / AI 모델 PoC) |
| 사업부 채택 | Flash YE | **핵심과제 등록** (인프라 전수 운영 / AI 모델 PoC) |

---

**최호길 책임은 "AI 기술 깊이 × 데이터 엔지니어링 × 반도체 도메인 놀리지" 의 3축 융합이라는 사내 일반 AI 엔지니어 단독으로는 어려운 차별성을 보유하고 있으며, 본 위원회에서 사내 AI Specialist 로 선정되기에 충분한 자격을 갖추고 있다고 판단된다.**
