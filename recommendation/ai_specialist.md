...
> **본 추천서의 성격 — 양산 운영 시스템 구축 보고서.** 본 추천서는 학술 논문 보고서가 아닌 **양산 운영 시스템 구축 보고서** 다. 모든 정량 성과는 (1) 실제 EDS test 데이터를 가공·제공하는 파이프라인 (fail-map), (2) 현업 엔지니어가 웹앱에서 자유롭게 분석·확인하는 도구 (mapviewer), (3) AI 기반 결함 분류·이상감지 모델 (known-cnn / unknown-contrastive / anomaly-detection) 의 3 축이 결합된 시스템에서 도출되었다. 따라서 본 추천서의 평가 기준은 "논문 IF" 가 아니라 **"양산 현장 엔지니어 업무 개선에 얼마나 직접 기여했는가"** 다.
...
| 정확도 (도메인 추정) | mAP@50 약 0.6-0.7 (Ultralytics 공식 README YOLO11m COCO mAP@50 약 0.66 기준, wafer 결함 chip 의 작은 크기 ~50×50 px @ 1024 wafer 특성상 small-object detection 일반 성능 저하 -0.1~0.15 적용한 추정. small-object 의 일반 성능 저하는 He et al. 2017 Mask R-CNN, Lin et al. 2017 FPN 등에서 보고된 -0.1 ~ -0.15 mAP 감소를 본 wafer chip 크기에 적용한 값이다. 본 PoC 단계에서 직접 비교 측정 예정) | chip CNN val_f1 1.000 / test_f1 0.9799 (실측) |
...
ConvNeXtV2-Base FCMAE 백본 선택은 본인의 도메인 통찰 + **전해곤 교수 자문 (연세대 인공지능 컴퓨팅 학부, 이미지 분류 전공)** + 사내 ablation 의 3 축 결정이다. 자문 요지는 (1) wafer fail 분포가 center / edge / ring 등 **국소 영역 패턴** 으로 분포 → global attention (ViT) 보다 **local 특징이 강한 CNN** 이 적합, (2) ViT / Transformer 계열은 학습에 매우 큰 데이터셋이 필요한데 본 도메인의 label 개수가 부족 → ConvNeXtV2 같은 CNN baseline 이 현업 조건에 맞다. 실측에서도 ConvNeXtV2-Base 가 MaxViT (119.5M) 대비 동일 F1 0.87 을 88.6M (−26%) 으로 달성하며 FLOPs 도 39% 적었다. **즉 본 백본 결정은 본인 도메인 통찰 (wafer fail 의 local 패턴 + label 부족) 과 전해곤 교수 학계 자문 (CNN ↔ ViT 적합성) 과 사내 ablation 실측 (88.6M / -26% / -39% FLOPs) 세 축이 모두 일치하는 결정이다.**
...
| 인증 | **SAML / OneLogin SSO** |
| RBAC | **USER / POWER / ADMIN / SUPER 4 단계** |
| 실사용 로그 | **stats.json 2,317 요청 / access.log 6.9 MB** |
...
1. **13년 도메인 + AI 기술 + 데이터 엔지니어링의 한 사람 결합.** Photo 10년 (CD overlay trend, overlay bit data 분석) + QIE 3년 (L1 trend) 의 도메인 경력 위에 AI 모델 · 합성 · 평가의 의사결정을 직접 코드화한 결과물이 본 3 과제다. 구체 매핑은 3.3 / 3.4 / 9.0 표 참조.
...
## 9.5 학술 논문 인용 매핑 (External Academic References)

본 추천서의 3 과제 (P1 / P2 / P3) 에서 사용·검토된 모든 AI 기법은 원문 학술 논문에 기반한다. 본 절은 추천서의 모든 기법 결정이 **학계에서 검증된 표준 기법**위에 도메인 통찰 + 자체 변형을 더한 것임을 보이기 위한 외부 인용 매핑이다. 위원회 / 외부 검토자가 기법의 출처와 신뢰도를 빠르게 확인할 수 있도록 정리한다.

### 9.5.1 Backbone & Pretraining

| 기법 | 원문 인용 | 추천서 사용 위치 |
|------|-----------|-------------------|
| **ConvNeXtV2** | Woo, S., Debnath, S., Hu, R., Chen, X., Liu, Z., Kweon, I.S., Xie, S. "ConvNeXt V2: Co-designing and Scaling ConvNets with Masked Autoencoders." *CVPR 2023*. arXiv:2301.00808. | P1 (Base, backbone), P2 (backbone), P3 (Tiny) 공통 |
| **FCMAE** (Fully Convolutional Masked Autoencoder) | 동일 논문 (Woo et al. CVPR 2023). | P1 / P3 TAPT pretraining |
| **MaxViT** | Tu, Z., Talebi, H., Zhang, H., Yang, F., Milanfar, P., Bovik, A., Li, Y. "MaxViT: Multi-Axis Vision Transformer." *ECCV 2022*. arXiv:2204.01697. | P1 백본 비교 대상 |
| **ViT** (Vision Transformer) | Dosovitskiy, A., Beyer, L., Kolesnikov, A., Weissenborn, D., et al. "An Image is Worth 16x16 Words: Transformers for Image Recognition at Scale." *ICLR 2021*. arXiv:2010.11929. | P1 백본 비교 대상 (학계 자문 인용 — 전해곤 교수 의견의 비교축) |
| **ResNet** | He, K., Zhang, X., Ren, S., Sun, J. "Deep Residual Learning for Image Recognition." *CVPR 2016*. arXiv:1512.03385. | CNN 계열 비교 baseline |

### 9.5.2 Self-Supervised / Contrastive Learning (P1 unknown-contrastive)

| 기법 | 원문 인용 | 추천서 사용 위치 |
|------|-----------|-------------------|
| **SimCLR** | Chen, T., Kornblith, S., Norouzi, M., Hinton, G. "A Simple Framework for Contrastive Learning of Visual Representations." *ICML 2020*. arXiv:2002.05709. | P1 unknown-contrastive baseline |
| **MoCo v2** | Chen, X., Fan, H., Girshick, R., He, K. "Improved Baselines with Momentum Contrastive Learning." arXiv:2003.04297, 2020. (원본: He, K., Fan, H., Wu, Y., Xie, S., Girshick, R. "Momentum Contrast for Unsupervised Visual Representation Learning." *CVPR 2020*. arXiv:1911.05722.) | P1 Queue mechanism |
| **SupCon** (Supervised Contrastive) | Khosla, P., Teterwak, P., Wang, C., Sarna, A., Tian, Y., Isola, P., Maschinot, A., Liu, C., Krishnan, D. "Supervised Contrastive Learning." *NeurIPS 2020*. arXiv:2004.11362. | P1 대안 검토 (known label 부분 활용 가능성) |
| **DenseCL** | Wang, X., Zhang, R., Shen, C., Kong, T., Li, L. "Dense Contrastive Learning for Self-Supervised Visual Pre-Training." *CVPR 2021*. arXiv:2011.09157. | P1 Local InfoNCE (dense patch contrastive) |
| **NeCo** | Pariza, V., Salehi, M., Burghouts, G., Locatello, F., Asano, Y.M. "NeCo: Improving DINOv2's spatial representations in 19 GPU hours with Patch Neighbor Consistency" (also: "Near, far: Patch-ordering enhances vision foundation models' scene understanding"). arXiv:2408.11054, 2024. | P1 patch neighbor consistency loss 참조 |
| **InfoNCE** | van den Oord, A., Li, Y., Vinyals, O. "Representation Learning with Contrastive Predictive Coding." arXiv:1807.03748, 2018. | P1 contrastive loss 기본 식 |
| **NV-Retriever** (hard-negative filtering) | Moreira, G. de S.P., Osmulski, R., Xu, M., Ak, R., Schifferer, B., Oldridge, E. "NV-Retriever: Improving text embedding models with effective hard-negative mining." arXiv:2407.15831, 2024. | P1 NEG filter (positive-aware false-negative 제거) 영감원 |

### 9.5.3 Clustering & 평가 지표 (P1 unknown 평가)

| 기법 | 원문 인용 | 추천서 사용 위치 |
|------|-----------|-------------------|
| **HDBSCAN** | Campello, R.J.G.B., Moulavi, D., Sander, J. "Density-Based Clustering Based on Hierarchical Density Estimates." *PAKDD 2013*. (가속 구현: McInnes, L., Healy, J. "Accelerated Hierarchical Density-based Clustering." *ICDMW 2017*.) | P1 unknown cluster 추출 |
| **ARI** (Adjusted Rand Index) | Hubert, L., Arabie, P. "Comparing Partitions." *Journal of Classification* 2(1), 193-218, 1985. | P1 unknown 평가 지표 (0.859 ± 0.018) |
| **AMI** (Adjusted Mutual Information) | Vinh, N.X., Epps, J., Bailey, J. "Information Theoretic Measures for Clusterings Comparison: Variants, Properties, Normalization and Correction for Chance." *JMLR* 11, 2837-2854, 2010. | P1 unknown 평가 보조 지표 |
| **UMAP** | McInnes, L., Healy, J., Melville, J. "UMAP: Uniform Manifold Approximation and Projection for Dimension Reduction." *JOSS* 3(29), 861, 2018. arXiv:1802.03426. | P1 임베딩 시각화 |

### 9.5.4 Augmentation & Mixing (P2 chip multi-label)

| 기법 | 원문 인용 | 추천서 사용 위치 |
|------|-----------|-------------------|
| **CutMix** | Yun, S., Han, D., Oh, S.J., Chun, S., Choe, J., Yoo, Y. "CutMix: Regularization Strategy to Train Strong Classifiers with Localizable Features." *ICCV 2019*. arXiv:1905.04899. | P2 FCM-PM 핵심 합성 기법의 모태 (양자화 grade 보존) |
| **Mixup** | Zhang, H., Cisse, M., Dauphin, Y.N., Lopez-Paz, D. "mixup: Beyond Empirical Risk Minimization." *ICLR 2018*. arXiv:1710.09412. | P2 비교 대상 (학계 자문 인용 — 박은병 교수: 양자화 grade 손상되어 부적합) |
| **Label Smoothing** | Müller, R., Kornblith, S., Hinton, G. "When Does Label Smoothing Help?" *NeurIPS 2019*. arXiv:1906.02629. | P2 BCE + LS 안정화 |

### 9.5.5 Loss Functions (P2 / P3)

| 기법 | 원문 인용 | 추천서 사용 위치 |
|------|-----------|-------------------|
| **Focal Loss** | Lin, T.-Y., Goyal, P., Girshick, R., He, K., Dollár, P. "Focal Loss for Dense Object Detection." *ICCV 2017*. arXiv:1708.02002. | P3 class imbalance, P2 검토 |
| **ASL** (Asymmetric Loss) | Ridnik, T., Ben-Baruch, E., Zamir, N., Noy, A., Friedman, I., Protter, M., Zelnik-Manor, L. "Asymmetric Loss For Multi-Label Classification." *ICCV 2021*. arXiv:2009.14119. | P2 multi-label loss 검토 |
| **Knowledge Distillation** | Hinton, G., Vinyals, O., Dean, J. "Distilling the Knowledge in a Neural Network." *NeurIPS Deep Learning Workshop 2015*. arXiv:1503.02531. | P2 distill 검토 |

### 9.5.6 Optimizer / Scheduler / Normalization (공통)

| 기법 | 원문 인용 | 추천서 사용 위치 |
|------|-----------|-------------------|
| **AdamW** | Loshchilov, I., Hutter, F. "Decoupled Weight Decay Regularization." *ICLR 2019*. arXiv:1711.05101. | P1 / P2 / P3 공통 optimizer |
| **Cosine Annealing (SGDR)** | Loshchilov, I., Hutter, F. "SGDR: Stochastic Gradient Descent with Warm Restarts." *ICLR 2017*. arXiv:1608.03983. | P1 / P3 LR schedule |
| **BatchNorm** | Ioffe, S., Szegedy, C. "Batch Normalization: Accelerating Deep Network Training by Reducing Internal Covariate Shift." *ICML 2015*. arXiv:1502.03167. | CNN baseline 비교 시 사용 |
| **Mean Teacher / EMA** | Tarvainen, A., Valpola, H. "Mean Teachers are Better Role Models: Weight-averaged Consistency Targets Improve Semi-supervised Deep Learning Results." *NeurIPS 2017*. arXiv:1703.01780. | P3 EMA model averaging |

### 9.5.7 Object Detection (비교 대상 / 부차적 참고)

| 기법 | 원문 인용 | 추천서 사용 위치 |
|------|-----------|-------------------|
| **Mask R-CNN** | He, K., Gkioxari, G., Dollár, P., Girshick, R. "Mask R-CNN." *ICCV 2017*. arXiv:1703.06870. | small object detection 비교 |
| **FPN** | Lin, T.-Y., Dollár, P., Girshick, R., He, K., Hariharan, B., Belongie, S. "Feature Pyramid Networks for Object Detection." *CVPR 2017*. arXiv:1612.03144. | small object detection 비교 |
| **YOLOv11** | Ultralytics. YOLOv11 공식 구현 / 문서 (https://github.com/ultralytics/ultralytics, 2024). | P1 검출 비교 (mAP@50 ~0.66 벤치 참고) |

### 9.5.8 Generative (P2 학계 자문 인용 — 비교축)

| 기법 | 원문 인용 | 추천서 사용 위치 |
|------|-----------|-------------------|
| **DDPM (Diffusion)** | Ho, J., Jain, A., Abbeel, P. "Denoising Diffusion Probabilistic Models." *NeurIPS 2020*. arXiv:2006.11239. | P2 비교 대상 (학계 자문 — 픽셀값 보간으로 grade 손상되어 부적합) |
| **GAN** | Goodfellow, I., Pouget-Abadie, J., Mirza, M., Xu, B., Warde-Farley, D., Ozair, S., Courville, A., Bengio, Y. "Generative Adversarial Nets." *NeurIPS 2014*. arXiv:1406.2661. | 합성 데이터 생성 비교 baseline |

### 9.5.9 Time Series / Anomaly (P3)

| 기법 | 원문 인용 | 추천서 사용 위치 |
|------|-----------|-------------------|
| **AR(1)** | Box, G.E.P., Jenkins, G.M. *Time Series Analysis: Forecasting and Control.* Holden-Day, 1970. (이후 5판 Box, Jenkins, Reinsel, Ljung. Wiley, 2015) — classical reference. | P3 trend 합성 noise 모형 |
| **OU process** (Ornstein-Uhlenbeck) | Uhlenbeck, G.E., Ornstein, L.S. "On the Theory of the Brownian Motion." *Physical Review* 36, 823-841, 1930. | P3 baseline trend 생성 |

### 9.5.10 Hyperparameter Optimization & Dataset

| 항목 | 원문 인용 | 추천서 사용 위치 |
|------|-----------|-------------------|
| **Optuna** | Akiba, T., Sano, S., Yanase, T., Ohta, T., Koyama, M. "Optuna: A Next-generation Hyperparameter Optimization Framework." *KDD 2019*. arXiv:1907.10902. | P1 / P2 / P3 HPO |
| **WM-811K** (dataset) | Wu, M.-J., Jang, J.-S.R., Chen, J.-L. "Wafer Map Failure Pattern Recognition and Similarity Ranking for Large-Scale Data Sets." *IEEE Transactions on Semiconductor Manufacturing* 28(1), 1-12, 2015. DOI: 10.1109/TSM.2014.2364237. | P1 외부 검증용 공개 dataset |

### 9.5.11 추천서 인용 형식 요약 (Quick-Reference)

> **P1 백본:** ConvNeXtV2-Base + FCMAE pretraining [Woo et al. CVPR 2023]
> **P1 known-CNN:** 2-stage (wafer + chip) — 도메인 통찰 기반 자체 설계
> **P1 unknown-contrastive:** SimCLR [Chen et al. ICML 2020] InfoNCE [van den Oord et al. 2018] + MoCo Queue [Chen et al. arXiv:2003.04297, 2020] + DenseCL Local [Wang et al. CVPR 2021] + NeCo neighbor consistency [Pariza et al. arXiv:2408.11054, 2024] + NV-Retriever-style NEG filter [Moreira et al. arXiv:2407.15831, 2024]
> **P1 평가:** HDBSCAN [Campello et al. PAKDD 2013; McInnes & Healy ICDMW 2017] + ARI [Hubert & Arabie 1985] + AMI [Vinh et al. JMLR 2010]
> **P2 합성:** CutMix [Yun et al. ICCV 2019] 기반 자체 변형 FCM-PM (Mixup [Zhang et al. ICLR 2018] / Diffusion [Ho et al. NeurIPS 2020] 은 학계 자문으로 제외)
> **P2 loss:** BCE + Label Smoothing [Müller et al. NeurIPS 2019] (ASL [Ridnik et al. ICCV 2021], Focal [Lin et al. ICCV 2017] 검토)
> **P3 백본:** ConvNeXtV2-Tiny + FCMAE [Woo et al. CVPR 2023]
> **P3 loss:** Focal Loss [Lin et al. ICCV 2017]
> **P3 schedule:** AdamW [Loshchilov & Hutter ICLR 2019] + Cosine Annealing [Loshchilov & Hutter ICLR 2017] + EMA [Tarvainen & Valpola NeurIPS 2017]
> **P3 합성:** AR(1) noise [Box & Jenkins 1970] + OU baseline [Uhlenbeck & Ornstein 1930] — 도메인 파라미터는 본인 photo 10년 측정 범위에서 도출
> **공통 HPO:** Optuna [Akiba et al. KDD 2019]
> **공개 dataset 검증:** WM-811K [Wu et al. IEEE TSM 2015]

> **요지:** 본 추천서의 모든 AI 기법 결정은 **(a) 학계 표준 기법 (위 인용 25+ 편) 위에**, **(b) 13년 photo / QIE 도메인 통찰을 자체 변형 (FCM-PM, 2-stage known-CNN, FBM alpha-함수, P3 episode 합성)으로 결합하고**, **(c) 연세대 전해곤 / 박은병 교수 학계 자문으로 외부 검증한** 3 축 의사결정의 결과물이다.
...
**최호길 책임의 핵심 자격: 13년 photo / QIE 도메인 경력 위에 AI 모델 · 데이터 엔지니어링 · 평가의 의사결정을 직접 코드화한 결과물이 본 3 과제다.** Known-CNN 2-stage 구조, Palette PNG 선택, P3 noise 3종 매핑, mapviewer + fail-map 운영 시스템 (106K 줄) — 본 추천서의 핵심 결정 (9.0 표 5 + 표 6) 은 모두 도메인 통찰을 코드까지 끌어내려 만든 산출물이며, 그 중 P1 백본 / P2 합성 기법의 핵심 결정은 **연세대 인공지능 컴퓨팅 학부 전해곤 / 박은병 교수의 학계 자문으로 외부 검증**도 받았다. 향후 2년 내 확장 예정인 multi-modal AI Orchestration Agent 는 (i) CD overlay trend 10년 + (ii) overlay bit data 10년 + (iii) AI / 딥러닝 구현 역량 + (iv) 사업부 인프라 운영 + (v) **연세대 석사과정 + 학계 자문 검증** 의 5가지 조건이 한 사람 안에 있어야 끌고 갈 수 있는 과제다. 본 위원회에서 사내 AI Specialist 로 선정되기에 충분한 자격을 갖추고 있다고 판단된다.
