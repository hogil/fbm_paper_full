...
**Abstract** - **반도체 EDS(Electrical Die Sorting) 결과를 공간적으로 보존한 Failbit Map은 수율 진단에 필요한 자료지만, 현업에서는 measure value와 제한된 map 조회에 의존하는 경우가 많아 반복 패턴과 신규 failure 검토가 늦어질 수 있다. 본 논문은 Failbit Map 이미지, chip 좌표, chip measure value, 모델 출력, 검토 라벨을 하나의 좌표 보존형 wafer 단위로 정렬한 분석 파이프라인을 다룬다. Known fail은 ConvNeXtV2-Base와 저신뢰 표본 한정 ROI(Region of Interest)-YOLO(You Only Look Once) 2-stage로 field validation weighted F1 0.95를 확인했고, object-id map은 generated-chip development extension으로 분리하였다. Unknown fail은 대조 학습과 HDBSCAN으로 운영 wafer를 13개 candidate group으로 압축했으며, 이 가운데 7개가 신규 failure로 확인되었다. 이 13→7은 분류 정밀도가 아니라 field review 후보 압축 결과다. chip 수준에서는 FCM-PM(Full-Cover CutMix + Pair Mask)을 적용해 controlled synthetic benchmark에서 micro-F1(bit-F1) 0.9927과 false alarm 0.00%를 확인하였다. 이 구성에서 Failbit Map은 사후 그림이 아니라 검토, 라벨 되먹임, 재학습에 다시 쓰이는 좌표 보존 자료로 남는다.**
...
![DRAM cell에서 좌표 보존 Failbit Map까지](figures/full_paper_rev185/fig01_dram_to_fbm.png)
...
본 논문은 대량 Failbit Map 생성의 기술 한계와 수작업 판정 의존 문제를 같은 좌표계의 데이터 파이프라인 위에서 다룬다. 작업 범위는 다섯 항목으로 나누었다. 첫째, Failbit Map·chip 좌표·chip measure value를 같은 좌표계에 맞춘 coordinate-preserving FBM data asset이다. 둘째, ConvNeXtV2-Base와 선택적 ROI-YOLO로 Known fail field validation weighted F1 0.95를 확인하고, Unknown fail은 자기지도 대조 학습과 HDBSCAN으로 13→7 candidate compression을 수행한 wafer-level analysis loop이다. 셋째, known chip coordinate를 쓰는 object-id map과 single 불량 chip만으로 2-combo를 학습하는 FCM-PM이다. 넷째, 검색·map view·chip overlay·measure 확인·label 관리를 통합한 web application review layer이다. 다섯째, operation impact, field validation, field review, generated development, controlled synthetic benchmark를 합산하지 않는 validation-scope separation이다. 이 항목들은 data asset layer(1) / analysis layer(2,3) / operation layer(4,5)의 3층으로 배치된다.

논문의 초점은 개별 모델 성능 경쟁보다 Failbit Map을 반복 검토 가능한 좌표 보존 자료로 만드는 데 있다. 기술적으로는 두 지점을 따로 확인하였다. 첫째, categorical object-id를 자연 해상도 one-hot으로 인코딩하면 fixed chip coordinate라는 반도체 prior 때문에 localization이 제거되고, categorical spatial representation이 형성된다(측정 근거는 2.2.1). 이 결과는 모델 용량 우열이 아니라 입력 표현의 차이로 해석한다. 둘째, 정답 2-combo 라벨이 없는 조건에서 FCM-PM을 사용해 단일 불량 chip으로 중복 불량 학습 신호를 만들었고, Pair Mask가 합성 background의 false alarm을 줄이는 원인임을 ablation으로 확인하였다(2.2.3). object-id map은 미양산 generated-chip development result다(2.2.1).

### 1.1. 기술 개요

설계할 때 가장 먼저 둔 조건은 웨이퍼의 공간 의미를 손상시키지 않는 것이었다. 무손실 palette 표현, flip 증강 배제, 국소 수용장 backbone과 선택적 2-stage 검증, 격자 고정 structured sampling, categorical one-hot 인코딩은 모두 이 조건에서 나온다. RGB 대신 palette, flip 대신 배제, Mixup 대신 영역 보존 CutMix, SwAV random-crop 대신 grid 고정, 정수 보간 대신 one-hot을 택했다. Table 1은 이를 데이터 표현, Known 판정, chip 단위 object-id map, Unknown 후보 압축, 검토 되먹임으로 정리한다. Unknown 경로의 InfoNCE(Information Noise-Contrastive Estimation)는 라벨 없이 같은 wafer는 가깝게 다른 wafer는 멀게 임베딩을 학습하는 대조학습 목적함수이고, HDBSCAN(Hierarchical Density-Based Spatial Clustering of Applications with Noise)[7]은 밀도 군집과 noise를 함께 분리하는 군집화 방법이다.
...
![좌표 보존 분석 파이프라인](figures/full_paper_rev185/fig02_pipeline_architecture.png)
...
웨이퍼 맵 불량 분석은 규칙 기반·고전 ML을 거쳐 CNN 분류로 발전해 왔다. 공개 데이터셋 WM-811K를 CNN으로 분류한 연구 [1][2]는 die 단위 wafer-level 패턴 분류에 중심을 두고, wafer-level 라벨만으로 chip 불량을 추정하는 [18]도 chip 형태를 좌표에 복원하지는 않는다. 본 논문의 object-id map은 chip 분류 id를 wafer 격자 제자리에 categorical로 되돌려 형태(morphology)와 위치를 함께 보존한다. 한 chip에 둘 이상이 겹치는 혼합형 불량은 bin map에서 CNN으로 분류된 바 있으나 [19] 이는 wafer 단위 혼합 패턴을 실제 mixed 라벨로 학습한다. 여기서는 chip 단위에서 단일 불량만으로 2-combo를 합성 학습(FCM-PM)해 mixed 라벨 부족을 다룬다. 라벨 없는 신규 패턴 탐색은 대조 학습·군집화 [3][6], open-set recognition [20], 대조 deep clustering [21], open-world 군집화·decision fusion [4]로 이어졌고 대개 공개 bin map에서 ARI(Adjusted Rand Index)·NMI(Normalized Mutual Information)·silhouette로 평가된다. 본 논문의 Unknown 경로도 대조-군집 계열을 쓰지만, 정답 라벨이 없는 양산 운영의 좌표 보존 FBM에서 동작하므로 합성 평가셋에서만 이 관례를 따르고 실전에서는 후보 압축(13→7)으로 검증하였다.
...
![ROI-YOLO 2-stage 검출](figures/full_paper_rev185/fig03_roi_yolo.png)
...
![object-id map 생성 흐름](figures/full_paper_rev185/fig04_objid_flow.png)
...
ROI-YOLO와 object-id map의 Stage-2 구조·연산 효율을 Table 2에 비교하였다. ROI-YOLO는 ROI 추출·box 회귀·NMS를 수행하며 512px 이상 입력을 요구한다(공개 detector YOLO11x 기준 56.9M params·194.9 GFLOPs@640 [22]; ROI 한정 crop의 실제 연산은 이보다 작다). object-id map은 chip 좌표로 확정된 32×32 격자를 1.16M chip-CNN에 입력하며, 아키텍처 산출 연산량은 약 0.31 GFLOPs다.[^1] 입력 스케일(32×32 vs 640)이 다르므로 이는 같은 조건의 속도 비교가 아니라 좌표 보존 표현이 입력 자체를 chip 격자로 줄인 결과로 보았다. 정확도도 생성 평가셋 기준 val 0.9946 / test 0.9872로, ROI-YOLO field weighted F1 0.95와 직접 비교하지 않았다.

비교의 목적은 모델 크기 우열이 아니라 문제 정의의 차이를 보이는 데 있다. ROI-YOLO는 위치를 모르는 detection 문제를 풀지만, wafer chip 좌표는 product layout으로 이미 정해져 있다. object-id map은 이 좌표를 사용해 localization을 생략하고, 각 chip을 한 번 crop·분류한 뒤 class id를 격자에 되돌린다. full wafer image에서는 local chip failure가 작은 영역으로 희석될 수 있지만, chip crop에서는 모델이 local failure morphology를 직접 관찰한다. 이후 class id를 원래 grid에 되돌리면 class별 공간 패턴이 categorical map으로 남는다. 따라서 결과는 작은 모델이 큰 detector를 대체했다는 뜻이 아니라, fixed chip coordinate와 natural-resolution one-hot representation이 이 데이터에 맞는 입력 형태였다는 의미로 해석한다.
...
![chip 라벨 공간 예시](figures/full_paper_rev185/fig05_chip_label_space.png)
...
![val_margin selection and NB-reject sidecar](figures/full_paper_rev185/fig07_probability_control.png)
...
추가 결합 구조의 필요성은 두 모델의 disagreement로 보았다. R-only backbone과 obj-id 두 모델의 동시 오답은 합성 평가셋에서 0.78%에 그쳤고, 대부분을 obj-only 단독이 이미 확보해 mid-fusion·cross-attention 여지는 작았다. 학습 비용도 R-only 88M의 약 12.5시간 대비 1.16M chip-CNN은 1분 미만으로 파라미터 약 76배·학습 약 750배 차이다. 비채택 in_ch=6도 seed 5회 0.9838±0.0092로 일관돼 입력 표현 차이를 다시 확인하였다. raw Failbit Map에서 구분이 어려운 두 클래스도 object-id map에서는 chip class 색 패턴으로 분리된다(Figure 8).

![raw와 object-id map 구분력 비교](figures/full_paper_rev185/fig_objid_compare.png)
...
본 논문은 wafer 이미지, chip 좌표, measure value, 모델 출력, 검토 라벨을 같은 좌표계에 묶어 Failbit Map을 반복 검토 가능한 자료로 구성하였다. Known 경로에서는 ConvNeXtV2와 선택적 ROI-YOLO로 field validation weighted F1 0.95를 얻었고, object-id map은 known chip coordinate로 localization을 줄이는 generated development extension으로 두었다. Unknown 경로에서는 라벨 없는 운영 wafer를 13개 후보군으로 압축했고, 그중 7개 신규 불량 group이 field review에서 확인되었다. chip 수준에서는 FCM-PM이 단일 불량 chip만으로 2-combo 학습 신호를 만들고 Pair Mask로 false alarm을 억제하여 controlled synthetic benchmark에서 bit-F1 0.9927과 FAR 0.00%를 보였다. 운영 측면에서는 viewer/data pipeline이 약 48매 단위 조회를 일 약 2만 wafer 누적 비교로 확장했으며, Flash YE 확산 요청은 별도 적용 검토 범위로 분리하였다.
