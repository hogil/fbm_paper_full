...
**Abstract** - **Failbit Map은 EDS(Electrical Die Sorting) 검사 결과를 wafer 좌표에 남기는 자료로, 불량의 위치와 형태를 함께 보여 준다. 그러나 실제 분석에서는 chip별 measure value와 제한된 map 조회에 의존하는 경우가 많아 반복 패턴이나 신규 failure를 늦게 발견할 수 있다. 이 논문에서는 Failbit Map 이미지, chip 좌표, chip measure value, 모델 출력, review label을 같은 wafer 좌표계에 맞춰 저장하고, 이를 Known 분류, Unknown 후보 압축, chip-level multi-label 검증에 함께 사용했다. Known fail은 ConvNeXtV2-Base와 저신뢰 표본 한정 ROI(Region of Interest)-YOLO(You Only Look Once) 2-stage로 field validation weighted F1 0.95를 확인했다. Unknown fail은 대조학습 embedding과 밀도 군집화로 운영 wafer를 13개 후보 group으로 줄였고, 이 중 7개가 현업 검토에서 신규 failure로 확인되었다. 이 13→7은 분류 정밀도가 아니라 field review 후보 압축 결과다. chip 수준에서는 single-failure chip만으로 2-combo를 학습하기 위해 FCM-PM(Full-Cover CutMix + Pair Mask)을 설계했으며, controlled synthetic benchmark에서 micro-F1(bit-F1) 0.9927과 false alarm 0.00%를 얻었다. 각 결과는 field validation, field review, generated development, controlled synthetic benchmark로 분리해 해석했다. 이 구조에서 Failbit Map은 단순 조회용 이미지가 아니라 review, label feedback, retraining에 다시 쓰이는 좌표 보존 자료가 된다.**
...
![DRAM cell에서 좌표 보존 Failbit Map까지](figures/full_paper_rev186/fig01_dram_to_fbm.png)
...
현장 분석은 오랫동안 measure value를 축으로 이루어졌다. 담당자는 chip별 수치로 fail 여부를 확인하고, 수식과 임계값을 직접 조합해 고질 불량을 분류해 왔다. 방식은 rule 기반이라 근거가 분명했지만, 사실상 사람이 직접 만든 판정 모델에 가까웠다. 의심스러운 wafer는 Failbit Map이나 TEM(Transmission Electron Microscopy)으로 시료를 잘라 CD(Critical Dimension)나 miss-alignment 같은 pattern을 확인하고 공정 structure와 layout 원인을 해석했다. 이 방식은 수동 임계값에 의존해 누락 위험이 있고, chip을 단일 수치로 환원하는 탓에 주변 chip 관계나 반복 pattern 같은 공간 정보를 놓칠 수 있다. 기존 도구로는 한 번에 약 48매까지만 조회할 수 있어 전수 검토도 어려웠다. 원인 해석에는 Failbit Map 확인이 사실상 필수였지만, 대용량 raw log 처리, 이미지 생성 속도, 저장 용량, chip 좌표 정합 같은 기술 문제로 Failbit Map을 반복 검토 가능한 자료로 만들기는 쉽지 않았다.
...
이 논문은 두 문제에서 출발했다. 하나는 Failbit Map을 대량으로 만들고 볼 수 있어야 한다는 점이고, 다른 하나는 사람이 정한 임계값만으로는 신규 패턴을 놓칠 수 있다는 점이다. 그래서 raw log를 좌표 보존 이미지와 chip annotation으로 바꾸고, 그 위에 Known 분류와 Unknown 후보 압축을 연결했다. chip 수준에서는 known chip coordinate를 쓰는 object-id map과 single 불량 chip만으로 2-combo를 학습하는 FCM-PM을 별도로 두었다. 현업 review층은 검색, map view, chip overlay, measure 확인, label 관리를 맡는다. 검증 범위는 operation impact, field validation, field review, generated development, controlled synthetic benchmark로 나누어 합산하지 않았다. 정리하면 데이터 적재층, 모델 분석층, 현업 review층의 세 단계로 볼 수 있다.

개별 모델의 성능 경쟁보다 Failbit Map을 반복 검토 가능한 좌표 보존 자료로 만드는 일을 우선했다. 기술적으로는 두 지점을 따로 확인하였다. 첫째, categorical object-id를 자연 해상도 one-hot으로 인코딩하면 fixed chip coordinate라는 반도체 prior 때문에 localization이 줄고, categorical spatial representation이 남는다(측정 근거는 2.2.1). 이 결과는 모델 용량 우열이 아니라 입력 표현의 차이로 해석한다. 둘째, 정답 2-combo 라벨이 없는 조건에서 FCM-PM을 사용해 단일 불량 chip으로 중복 불량 학습 신호를 만들었다. Pair Mask가 합성 background의 false alarm을 줄이는 원인이라는 점은 ablation으로 확인하였다(2.2.3). object-id map은 미양산 generated-chip development result다(2.2.1).

### 1.1. 기술 개요

설계할 때 가장 먼저 둔 조건은 웨이퍼의 공간 의미를 손상시키지 않는 것이었다. 그래서 저장은 RGB가 아니라 palette로 했고, 방향 의미가 바뀌는 flip 증강은 쓰지 않았다. backbone은 국소 수용장과 선택적 2-stage 검증을 기준으로 골랐다. Unknown 학습에서는 random crop 대신 격자를 고정했고, object-id 입력은 정수 보간 대신 categorical one-hot으로 두었다. Table 1은 이를 데이터 표현, Known 판정, chip 단위 object-id map, Unknown 후보 압축, 검토 되먹임으로 정리한다. Unknown 경로의 InfoNCE(Information Noise-Contrastive Estimation)는 라벨 없이 같은 wafer는 가깝게 다른 wafer는 멀게 임베딩을 학습하는 대조학습 목적함수이고, HDBSCAN(Hierarchical Density-Based Spatial Clustering of Applications with Noise)[7]은 밀도 군집과 noise를 함께 분리하는 군집화 방법이다.
...
![좌표 보존 분석 파이프라인](figures/full_paper_rev186/fig02_pipeline_architecture.png)
...
이 논문에서는 같은 좌표계 안에서 세 항목을 함께 다뤘다. 첫째, chip 형태를 object-id map으로 다시 wafer 위치에 놓는다. 둘째, 실제 mixed 라벨 없이 FCM-PM으로 chip-level 2-combo 학습 신호를 만든다. 셋째, 라벨 없는 운영 wafer에서는 정량 분류 metric 대신 field review 후보 압축(13→7)으로 결과를 확인한다. 모델 용량보다 입력 표현이 성능을 좌우한다는 [13]의 관점에서 보면, categorical 신호에는 정수 보간보다 자연 해상도 one-hot이 더 적합했다(정량 근거 2.2.1). 2-stage 보정은 coarse-to-fine cascade 검출 [10][12]을 wafer의 chip 단위 class 검증에 맞게 재구성한 것이다. ConvNeXtV2, YOLO, InfoNCE, HDBSCAN 자체는 기성 방법이지만, 좌표 의미를 보존한다는 제약을 기준으로 palette 표현, flip 배제, backbone, structured sampling, one-hot 인코딩을 선택한 점이 기존 공개 benchmark 중심 연구와 다르다.
...
![ROI-YOLO 2-stage 검출](figures/full_paper_rev186/fig03_roi_yolo.png)
...
![object-id map 생성 흐름](figures/full_paper_rev186/fig04_objid_flow.png)
...
object-id map의 목적은 더 작은 모델로 detector를 이기는 것이 아니다. ROI-YOLO는 위치를 모르는 물체를 찾는 detector라서 후보 box를 찾고, box를 보정하고, NMS로 후보를 정리한다. 하지만 wafer의 chip 위치는 제품 layout으로 이미 정해져 있다. 이 점을 이용하면 Stage 2 문제를 detection이 아니라 chip별 fixed-crop classification으로 바꿀 수 있다. 각 chip을 한 번 잘라 class를 예측하고, 그 결과를 다시 wafer grid에 놓으면 object-id map이 된다. full wafer image에서 작게 보이던 chip-level failure도 crop 안에서는 충분히 큰 입력이 된다. 따라서 object-id map의 이득은 작은 모델이 큰 detector를 이겼다는 뜻이 아니라, fixed chip coordinate를 활용한 representation prior의 효과로 해석한다.
...
이 구조의 설계 전제는 categorical 신호 보존이다. 정수 object-id를 BICUBIC으로 보간하면 1.3, 2.7 같은 무의미한 실수가 생겨 신호가 손상되므로, 격자 해상도는 obj-id의 자연 해상도(제품별 chip 격자 수)로 유지했다. one-hot이 categorical 입력에 적합하다는 원칙 [11]에 따라 obj-id를 5채널 one-hot으로 인코딩하고(R/G/B 픽셀값 제외) 정수 block-expand만 적용해 BICUBIC 대비 복원 오차가 약 75% 줄었다(이 75%는 복원 오차로, palette 재구성의 파일 크기 약 75% 절감과는 별개 측정값이다). 이 표현은 정수 압축의 ordinal 오류와 단일 class 이진 표시의 정보 손실을 모두 피하며, R을 뺀 단독(in_ch=5)을 채택해 정확도·입출력 비용을 함께 줄였다(정량 비교 2.2.1).
...
![chip 라벨 공간 예시](figures/full_paper_rev186/fig05_chip_label_space.png)
...
2-combo chip label을 실제로 충분히 모으기는 어렵다. 그래서 단일 failure chip source를 활용해 두 failure가 함께 있는 상황을 만들었다. FCM-PM은 두 single-failure chip을 full-cover 방식으로 합쳐 A+B mixed chip을 만들고, 동시에 A-only와 B-only masked view를 함께 학습한다(Figure 6). mixed view는 두 failure가 함께 있을 때의 positive signal을 주고, masked view는 합성 과정에서 생긴 background가 false-positive signal로 학습되는 것을 막는다. false alarm rate(FAR)는 negative 중 오검출 비율(FP_neg/N_neg)로 계산하고, checkpoint는 validation에서 val_margin = mean(p_pos) - max(p_neg)이 가장 큰 epoch로 선택한다.

모델 출력은 4-bit sigmoid 확률이므로, 각 class의 positive와 negative 확률 분포를 따로 보아야 한다. ASL[16]만으로는 negative 억제가 강해 일부 weak-positive bit까지 같이 눌렸다. target을 positive 0.85 / negative 0.15로 분리했을 때가 더 안정적이었다. single-only 학습에서는 2-combo의 둘째 bit 평균이 약 0.42까지 낮았다. FCM 합성 후에는 이 값이 약 0.54로 올라갔다. PM은 이 과정에서 synthetic background가 negative-tail false alarm으로 학습되는 것을 막는다. 이후 val_margin은 validation에서만 positive/negative 확률 간격이 넓은 checkpoint를 고르는 기준으로 사용하였다(Figure 7a). Gaussian Naive Bayes reject는 최대 bit 하나가 아니라 4-bit probability shape 전체로 OOD tail을 걸러낸다(Figure 7b). bit 다수결 ensemble과 Knowledge Distillation(KD)[17] student는 각각 upper-bound와 추가 경량화 후보로 두었다.

![FCM-PM augmentation](figures/full_paper_rev186/fig06_fcm_pm.png)
...
![val_margin selection and NB-reject sidecar](figures/full_paper_rev186/fig07_probability_control.png)
...
전체 경로는 분석 UI에서 하나로 모인다. 뷰어는 피라미드 타일과 디스크 캐시로 약 1천만 픽셀 map의 zoom/pan을 가속하고, chip overlay 기반 BIN/FTN/QTN 시각화와 composite map으로 패턴 분류 결과 위에 계측값을 중첩한다. 검토 결과는 불량 등록과 학습 레이블로 되먹임되어 데이터 적재부터 재학습까지 하나의 운영 흐름으로 묶인다.
...
![raw와 object-id map 구분력 비교](figures/full_paper_rev186/fig_objid_compare.png)
...
구축한 viewer/data pipeline은 2025년부터 DRAM D1a~D1d 관련 제품 파트의 FBM review infrastructure로 사용된다. 여기서 사용 범위는 FBM 생성, 조회, overlay, label review를 지원하는 review infrastructure에 한정되며, Known/Unknown/chip model serving이나 object-id map 양산 적용 완료를 의미하지 않는다. 1시간 주기 자동 적재로 기존 도구의 한 번에 약 48매 열람이 일 약 2만 wafer 누적 비교로 확장되었다(Table 8). 운영에서 13개 후보 중 7개가 신규 불량으로 확인되어 검토에서 라벨, 재학습으로 이어지는 폐루프가 동작했다(Figure 9). 이 전환의 운영 효과는 분석 공수 약 90% 절감, P3WN 제품 사례의 수율 약 0.02%p 개선, 메모리제조기술센터 내부 성과 인정 기준 약 123억 규모로 집계되고 인정되었다.[^3] 또한 Flash YE에서 본 분석 구조의 확산 요청이 접수되어, Flash/NAND 계열 bin map review와 chip multi-label 분석으로의 적용을 검토 중이다. 다만 운영 정량 성과는 DRAM viewer/data pipeline 기준으로 한정한다. 이 값들은 viewer/data pipeline의 운영 효과이며, Table 4~7의 모델 metric과 합산하지 않는다.

![운영 web app(map viewer) 화면](figures/full_paper_rev186/fig09_webapp_viewer.png)
...
향후 연구는 네 축으로 제한한다. 첫째, DRAM YE팀과 object-id map 라벨 체계를 확정한 뒤 실제 데이터로 평가하고, Flash YE 확산 요청은 별도 적용 범위, 권한, 데이터 차이를 확인한 뒤 추진한다. 둘째, 같은 좌표계의 wafer 이미지와 chip 계측값을 융합해 패턴과 계측 근인을 함께 추적한다. 셋째, 대조 임베딩을 검색 인덱스로 재활용해 과거 유사 사례와 담당자 note를 찾는 진단 보조 기능을 검토한다. 넷째, 이미지와 trend, 이력 정보를 요약하는 large language model 기반 기능은 담당자 승인과 이력 관리가 있는 decision-support 범위로 한정한다.

## 3. 결론

이 논문에서는 wafer 이미지, chip 좌표, measure value, 모델 출력, 검토 라벨을 같은 좌표계에 묶어 Failbit Map을 반복 검토 가능한 자료로 구성하였다. Known 경로에서는 ConvNeXtV2와 선택적 ROI-YOLO로 field validation weighted F1 0.95를 얻었고, object-id map은 known chip coordinate로 localization을 줄이는 generated development extension으로 두었다. Unknown 경로에서는 라벨 없는 운영 wafer를 13개 후보군으로 압축했고, 그중 7개 신규 불량 group이 field review에서 확인되었다. chip 수준에서는 FCM-PM이 단일 불량 chip만으로 2-combo 학습 신호를 만들고 Pair Mask로 false alarm을 억제하여 controlled synthetic benchmark에서 bit-F1 0.9927과 FAR 0.00%를 보였다. 운영 측면에서는 viewer/data pipeline이 약 48매 단위 조회를 일 약 2만 wafer 누적 비교로 확장했으며, Flash YE 확산 요청은 별도 적용 검토 범위로 두었다.
