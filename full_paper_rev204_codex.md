...
![DRAM cell에서 좌표 보존 Failbit Map까지](figures/full_paper_rev204/fig01_dram_to_fbm.png)
...
Failbit Map 위에서 다루는 불량은 등록 failure(Known)와 아직 라벨 합의가 끝나지 않은 신규 후보(Unknown)로 나뉜다. 등록 failure는 16-class 라벨 체계 안에서 지도학습으로 다룰 수 있지만, 신규 후보는 바로 F1이나 recall로 평가하기 어렵다. 기존 분류기는 신규 패턴을 인접 class로 흡수하거나 낮은 신뢰도로 기각해 담당자에게 단서를 주지 못한다. 이 때문에 담당자는 Known 판정, 검토할 Unknown 후보 선별, 이미지와 chip 계측 근거 확인, 신규 라벨 등록 여부, 양산 noise와 진성 불량의 구분을 서로 다른 도구와 문서 사이에서 맞춰야 했다.

출발점은 현업에서 반복해서 부딪힌 두 문제였다. Failbit Map을 대량으로 만들고 볼 수 있어야 했고, 사람이 정한 임계값만으로는 신규 pattern을 놓칠 수 있었다. 실제 작업은 raw log를 palette PNG와 chip annotation으로 바꾸는 데서 시작했다. FTN, QTN, BIN measure는 같은 chip 좌표에 붙이고, 모델 출력과 review label도 wafer ID와 chip 위치로 다시 찾을 수 있게 했다. 이 자료 위에서 등록 failure를 판정하고, label이 없는 신규 후보는 검토군으로 정리했다. chip 수준에서는 이미 알고 있는 chip 좌표를 쓰는 object-id map과 single 불량 chip만으로 2-combo를 학습하는 FCM-PM을 별도 경로로 두었다. 담당자는 review 화면에서 wafer를 검색하고, map과 chip overlay를 보며, measure와 라벨을 같은 자리에서 확인한다. 운영에서 줄어든 일과 모델 평가에서 얻은 숫자는 같은 종류가 아니다. 그래서 Known F1, Unknown 13→7, object-id map, FCM-PM, 운영 효과는 Table 3에서 출처를 따로 표시했고 한 점수처럼 더하지 않았다.
...
![좌표 보존 분석 파이프라인](figures/full_paper_rev204/fig02_pipeline_architecture.png)
...
![ROI-YOLO 2-stage 검출](figures/full_paper_rev204/fig03_roi_yolo.png)
...
![object-id map 생성 흐름](figures/full_paper_rev204/fig04_objid_flow.png)
...
![chip 라벨 공간 예시](figures/full_paper_rev204/fig05_chip_label_space.png)
...
![FCM-PM augmentation](figures/full_paper_rev204/fig06_fcm_pm.png)
...
![val_margin selection and NB-reject sidecar](figures/full_paper_rev204/fig07_probability_control.png)
...
등록 failure 분류는 단계별로 성능을 분리해 관리하였다. 먼저 16-class closed-set(약 1,500 labeled, 4:1 stratified split)에서 backbone을 비교하였다(Table 4). MaxViT-T와 ConvNeXtV2-Base가 weighted F1 0.87로 같았으나 동일 split 단일 run이라 모델 효율을 기준으로 ConvNeXtV2-Base를 택했고, 단독 0.87에서 Optuna 탐색으로 validation weighted F1 0.92에 이르렀다.
...
![raw와 object-id map 구분력 비교](figures/full_paper_rev204/fig_objid_compare.png)
...
![운영 web app(map viewer) 화면](figures/full_paper_rev204/fig09_webapp_viewer.png)
