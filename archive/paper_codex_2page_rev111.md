...
![Fig. 1. Hex-to-grade conversion](_fig_hex_conversion.png)

**Fig. 1.** Hex-to-grade conversion. Cython optimization accelerated repeated conversion by about 100×.

Failbit Map은 제한된 색상 집합만 사용하므로 palette-indexed PNG를 적용하여 이미지 용량을 약 75% 절감하였다.

![Fig. 2. Palette-indexed PNG](_fig_palette_encoding.png)

**Fig. 2.** Palette-indexed PNG. Palette indexing reduced image size by about 75%.
...
**Fig. 3.** Known 불량 2단계 분류. CNN은 wafer-level center distribution만으로 B로 오분류했지만, ROI-YOLO가 chip pixel pattern을 검출해 A로 교정하였다.
...
본 연구는 대량 Failbit Map 생성 파이프라인과 Known·Unknown 불량 분석 AI를 통합 구현하였다. 데이터 계층에서는 1시간 주기 생성, 데이터 변환 속도 약 100배 향상, 이미지 용량 약 75% 절감을 달성하였고, Known 경로는 F1 0.95를, Unknown 경로는 운영 이미지에서 13개 후보 그룹 중 7개 실제 불량 그룹 검출을 확인하였다. 이는 Failbit Map 기반 불량 분석이 현업 운영 데이터에서도 실질적으로 동작함을 보여주며, 대량 생성과 자동 분석을 통합한 End-to-End 적용 체계를 검증한 결과이다.
