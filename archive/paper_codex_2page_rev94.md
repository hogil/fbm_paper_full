...
데이터 파이프라인은 설비 Log를 실시간 적재하고 1시간 주기로 Failbit Map을 생성하도록 구성하였다. 핵심 병목은 raw hex stream의 grade 변환 구간이다. 앞자리가 항상 0인 데이터 특성상 변환은 뒷자리 hex 문자만 처리하면 되지만, 이 단순 연산도 1 wafer당 평균 약 1,000만 회 반복되므로 Python 구현에서는 반복문과 분기 실행 오버헤드가 누적된다. 해당 루틴을 Cython으로 전환하여 동일 연산을 직접 수행함으로써 데이터 변환 속도를 약 100배 향상시켰다.
...
```text
    ...
    Python:
        c = s[2*i+1] -> to_int(c) -> 12 -> 3

    Cython:
        c = s[2*i+1] -> c - 'A' + 10 -> 12 -> 3
...
**Fig. 1.** Hex-to-grade conversion example. 앞자리가 항상 0이므로 뒷자리 문자만 처리하며, Python 구현은 문자 추출 후 변환 함수를 호출하는 반면 Cython 구현은 같은 문자를 직접 비교하고 산술 연산으로 변환한다. 이 차이는 1 wafer당 평균 약 1,000만 회 반복되면서 전체 처리 시간 차이로 누적된다.
