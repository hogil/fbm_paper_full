...
While the ConvNeXtV2-based wafer-level classifier showed high overall accuracy and throughput, its performance degraded on classes with similar wafer-level distributions of defective chips. To compensate for this, the second stage was configured so that ROI-based YOLO re-examines low-confidence samples and identifies chip-level defect morphologies and appearance patterns within the ROI (Fig. 3).
