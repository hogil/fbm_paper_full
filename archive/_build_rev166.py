...
# Reorder refs to match first-appearance order in body text (IEEE style).
# Order: FBM (2. intro) -> ConvNeXtV2 (2.2) -> MaxViT (2.2 C) -> SimCLR (2.3) -> HDBSCAN (2.3).
REFS_CODEX = [
    "[1] FBM, https://www.failbitmap.com",
    "[2] S. Woo et al., \"ConvNeXt V2: Co-Designing and Scaling ConvNets With Masked Autoencoders,\" "
    "in Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR), "
    "pp. 16133-16142, 2023.",
    "[3] Z. Tu et al., \"MaxViT: Multi-Axis Vision Transformer,\" in Proceedings of the European "
    "Conference on Computer Vision (ECCV), pp. 459-479, 2022.",
    "[4] T. Chen, S. Kornblith, M. Norouzi, and G. Hinton, \"A Simple Framework for Contrastive Learning "
    ...
    "[5] R. J. G. B. Campello, D. Moulavi, and J. Sander, \"Density-Based Clustering Based on Hierarchical "
...
KO_1_3 = (
    ...
    "둘째, Known 불량은 ConvNeXt V2 기반 1차 wafer-level 분류와 ROI 기반 YOLO 2차 분류를 결합한 2-stage 구조로 성능을 향상시켰다. "
    "셋째, Unknown 불량은 SimCLR 계열 self-supervised 분석과 grid-structured local sampling을 통해 보다 정밀하게 검출하도록 하였다."
)

KO_2_INTRO = "본 아키텍처는 데이터 파이프라인, Known 2-stage 분류기, Unknown self-supervised 검출기로 구성되며, 전용 분석 Web App[1]으로 배포되었다."
...
KO_2_1 = (
    ...
    "생성된 Map은 중앙 저장소에 적재되며 분석 Web App[1]으로 대량 조회·분석이 가능하다."
)

KO_2_2_A = (
    "Known 불량 분석은 16개 등록 클래스를 대상으로 하였으며, 1,500개의 Failbit Map을 사용하여 "
    "ConvNeXtV2[2] 기반 1단계 wafer-level 분류기와 저신뢰 샘플 대상 2단계 ROI(Region of Interest) 기반 YOLO를 결합한 2-stage 구조를 설계하였다."
)
...
KO_2_2_C = (
    "MaxViT[3]와 ConvNeXtV2 (Ref)는 동일한 weighted F1 0.87을 보였으나, ConvNeXtV2는 파라미터 수 약 26% 감소(119.5M → 88.6M)와 FLOPs 약 39% 감소(74.2G → 45.1G)로 더 효율적이라 최종 backbone으로 선정하였다. "
    "이후 Optuna를 통해 lr, weight decay, focal loss, class weight 등을 최적화하여 weighted F1을 0.92까지 향상시켰고, ROI-YOLO 보정으로 최종 0.95를 달성하였다."
)

KO_2_3_A = (
    "5일치 운영 데이터 10,000장을 사용하여 SimCLR[4] 계열 모델을 대조학습하였으며, "
    "wafer의 zone 기반 불량 해석 특성을 반영하기 위해 grid-structured local sampling을 함께 적용하였다. "
    "이후 별도 1일치 2,000장에 HDBSCAN[5]을 적용하여 유사 패턴을 그룹화하였다."
)
...
EN_1_3 = (
    ...
    "Second, for Known defects, we improved classification performance with a two-stage structure combining a ConvNeXt V2-based wafer-level first-stage classifier and an ROI-based YOLO second-stage classifier. "
    "Third, for Unknown defects, we applied a SimCLR-family self-supervised analysis with grid-structured local sampling for more precise detection."
)

EN_2_INTRO = "The architecture consists of a data pipeline, a two-stage Known classifier, and a self-supervised Unknown detector, deployed as a dedicated analysis web application[1]."
...
EN_2_1 = (
    ...
    "Generated maps are stored in a central repository and served through the analysis web application[1] for large-scale browsing and analysis."
)
...
EN_2_2_C = (
    "MaxViT[3] and ConvNeXtV2 (Ref) achieved the same weighted F1 of 0.87, but ConvNeXtV2 was selected as the final backbone because it was more efficient, with about 26% fewer parameters (119.5M \u2192 88.6M) and about 39% fewer FLOPs (74.2G \u2192 45.1G). "
...
EN_2_3_A = (
    "Using 10,000 production maps from five days, a SimCLR[4]-family model was trained with contrastive learning using grid-structured local sampling to reflect the zone-based nature of wafer defect interpretation. "
    "HDBSCAN[5] was then applied to a separate set of 2,000 maps from one additional day to group similar patterns."
)
...
if __name__ == "__main__":
    doc_ko = _build("ko")
    out_ko = OUT_DIR / "paper_codex_2page_rev166.docx"
    doc_ko.save(str(out_ko))
    print(f"saved: {out_ko}")

    doc_en = _build("en")
    out_en = OUT_DIR / "paper_codex_2page_rev166_en.docx"
