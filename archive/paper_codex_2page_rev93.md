...
```text
    ...
    Cal:
        grade = max(int(hex_pair, 16) - 9, 0)
```

**Fig. 1.** Hex-to-grade conversion example. 이 변환은 1 wafer당 평균 약 1천만 번 반복되므로, Python 구현에서는 문자열 파싱과 16진수 형변환이 병목이 되지만 Cython 구현은 byte 단위 접근과 typed loop를 통해 동일 연산을 더 빠르게 수행한다.
