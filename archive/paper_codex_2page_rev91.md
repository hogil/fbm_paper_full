...
데이터 파이프라인은 설비 Log를 실시간 적재하고 1시간 주기로 Failbit Map을 생성하도록 구성하였다. 핵심 병목은 raw hex stream의 grade 변환으로, 약 1,000만 pixel 반복 처리 때문에 Python 구현만으로는 처리량 확보가 어려웠다. 해당 루틴을 Cython으로 전환하여 데이터 변환 속도를 약 100배 향상시켰다.

**Hex-to-grade conversion**

$$
09090909090A \rightarrow 09\ 09\ 09\ 09\ 09\ 0A \rightarrow 9\ 9\ 9\ 9\ 9\ 10 \rightarrow 0\ 0\ 0\ 0\ 0\ 1
$$
...
**Fig. 1.** Raw hex-to-grade conversion example. Continuous hex stream is split into 2-character units, converted to decimal values, and corrected to final grade values.
...
$$
[(123,54,24), (123,54,24), \ldots, (123,54,24)],
$$
...
Baseline CNN의 weighted F1 0.78은 ConvNeXtV2-Base로 0.87, Optuna HPO로 0.92, ROI 기반 YOLO 보정으로 0.95까지 개선되었다. ROI 기반 YOLO는 전역 형상만으로 혼동되는 저신뢰 샘플의 오분류를 교정하였다(Fig. 3).

![Fig. 3. Known 불량 2단계 분류 예시](_fig_yolo_roi.png)

**Fig. 3.** Known 불량 2단계 분류 예시. 좌측 두 패널은 혼동되는 Class A/B 예시이며, 우측은 CNN이 B로 오분류한 샘플을 ROI-YOLO가 A로 교정한 사례를 보여준다.
