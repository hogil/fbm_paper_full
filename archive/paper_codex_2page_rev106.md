...
데이터 파이프라인은 설비 Log를 실시간 적재하고 1시간 주기로 Failbit Map을 생성하도록 구성하였다. 핵심 병목은 raw hex stream의 grade 변환 구간으로, 웨이퍼당 약 1,000만 pixel에 대해 반복 수행되므로 Python 구현만으로는 처리량 확보가 어려웠다. 해당 루틴을 Cython으로 최적화하여 데이터 변환 속도를 약 100배 향상시켰다.
...
**Fig. 1.** Hex-to-grade conversion example. Raw hex stream은 숫자 변환과 grade mapping을 거쳐 최종 grade row로 변환되며, Cython 최적화를 통해 반복 변환 성능을 약 100배 향상시켰다.

Failbit Map은 제한된 색상 집합만 사용하므로 palette-indexed PNG를 적용하여 이미지 용량을 약 75% 절감하였다.
...
**Fig. 2.** Palette-indexed PNG encoding example. Repeated RGB values are defined once in the palette, and each pixel stores only the palette index.

### Known 불량 분류

Known 불량 분석은 16개 기등록 클래스를 대상으로 하며, 총 1,500개의 라벨된 Failbit Map을 사용하였다. 표 1의 backbone 비교 결과 ConvNeXtV2-Base를 주 모델로 선정하였다.

| 구성 | 사전학습 | Test F1 | 비고 |
|---|---|---:|---|
| ViT-B/16 | IN-21k | 0.81 | fine-tune |
| Swin-B | IN-1k | 0.84 | fine-tune |
| EffNetV2-M | IN-1k | 0.85 | fine-tune |
| MaxViT-B | IN-21k | 0.87 | fine-tune |
| ConvNeXtV2-B | FCMAE+IN22k | 0.87 | selected |
| ConvNeXtV2-B + Optuna | FCMAE+IN22k | 0.92 | HPO |
| ConvNeXtV2-B + Optuna + ROI | FCMAE+IN22k | 0.95 | 2-stage |

전체 운영 기준 하루 약 2만 장 이상의 Wafer가 1시간 주기로 처리되므로, backbone 선택에서는 F1과 추론 처리량을 함께 고려하였다. MaxViT-Base와 ConvNeXtV2-Base는 동일한 Test F1 0.87을 보였으나, ConvNeXtV2-Base가 처리량 측면에서 더 유리하였다.

Baseline CNN의 weighted F1 0.78은 ConvNeXtV2-Base로 0.87, Optuna HPO로 0.92, ROI 기반 YOLO 보정으로 0.95까지 개선되었다. 특히 혼동되는 클래스는 wafer 내 위치보다 chip 내부 defect morphology 차이로 구분되므로, 저신뢰 샘플에 대해 ROI 기반 YOLO로 chip 내부 국소 pattern을 재판별하였다(Fig. 3).

![Fig. 3. Known 불량 2단계 분류 예시](_fig_yolo_roi.png)

**Fig. 3.** Known 불량 2단계 분류 예시. 두 클래스는 wafer-level 발생 영역은 모두 중심부로 유사하지만 ROI 내 chip morphology가 다르다. 우측 예시에서 1단계 CNN은 wafer-level pattern만으로 Class B로 오분류하였으나, ROI 기반 YOLO가 chip 내부 star pattern을 검출하여 최종적으로 Class A로 교정하였다.

### Unknown 불량 검출

Unknown 불량은 후보 grouping 문제로 정의하였다. 5일치 운영 데이터 약 10,000장으로 SimCLR 계열 contrastive learning 기반 임베딩을 학습하고, 별도 1일치 2,000장에 HDBSCAN을 적용하여 유사 패턴을 검출하였다. wafer의 zone 기반 불량 해석 특성을 반영하기 위해 grid structured local sampling을 적용하였으며, 그 결과 13개 불량 후보 그룹이 검출되었다. 현업 분석 엔지니어 검증 결과 이 중 7개가 실제 불량 그룹으로 판정되었다(Fig. 4).

![Fig. 4. Unknown 불량 grouping 예시](_fig_cluster.png)

**Fig. 4.** Unknown 불량 grouping 예시. 운영 이미지에서 검출된 13개 후보 그룹 중 현업 분석 엔지니어 검증 결과 7개가 실제 불량 그룹으로 판정되었다.

## 결론

본 연구는 대량 Failbit Map 운영을 위한 데이터 파이프라인과 Known·Unknown 불량 분석 AI를 통합 구현하였다. 데이터 계층에서는 1시간 주기 생성과 저장 효율을 확보하였고, Known 경로는 F1 0.95를, Unknown 경로는 13개 불량 후보 그룹 검출과 7개 실제 불량 그룹 판정을 확인하였다. 이를 통해 Failbit Map 기반 불량 분석의 현업 적용 가능성을 보였다.
