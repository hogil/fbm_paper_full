...
ABSTRACT_KO = (
    ...
    "데이터는 Cython 적용으로 변환 속도를 약 100배 향상시켰고, 이미지는 32-color palette-indexed PNG 적용으로 용량을 약 75% 절감하였다. "
...
ABSTRACT_EN = (
    ...
    "However, in practical operations, large-scale retrieval and inspection of Failbit Maps have been impractical, and even the limited analysis that was possible has depended heavily on manual inspection by engineers. "
    ...
    "After training on 10,000 Failbit Maps collected over five production days, when applied to 2,000 maps from an additional day the model detected 13 defect groups, "
    "of which 7 were confirmed as real defect groups by domain engineers, demonstrating practical applicability in production."
)
...
KO_1_2 = (
    ...
    "설비 Log는 Wafer당 10~50MB 수준이며 전 제품 기준 하루 약 2만 장의 Wafer가 생산되지만, 기존 환경에서는 속도와 메모리 제약으로 대량 처리가 어려웠고 엔지니어가 한 번에 조회·확인할 수 있는 수량도 48매 수준에 머물렀다. "
    "둘째, 생성된 Map에 대한 불량 여부 및 유형 판정이 엔지니어의 수동 판독에 의존하여 전수 분석이 어려웠다. "
    "본 논문은 이러한 한계를 해결하기 위해, 대량 Raw Data를 Failbit Map으로 생성·저장·조회하는 데이터 파이프라인과, Known 불량을 2-stage supervised classification으로 분석하고 Unknown 불량을 self-supervised 기반으로 검출하는 통합 분석 아키텍처를 제안한다. "
    "주요 기여는 다음과 같다."
)

KO_1_3 = (
    "첫째, 대량 설비 Log의 실시간 적재와 1시간 주기 Failbit Map 생성 체계를 구현하여, 대량 Map의 지속적 생성과 운영 활용이 가능한 데이터 처리 기반을 구축하였다. "
    "둘째, Known 불량 분류는 ConvNeXt V2로 Wafer 내 불량 Chip들의 종류와 분포 패턴을 1차 분류하고, 유사 분포로 인해 혼동되는 샘플은 ROI 기반 YOLO로 개별 Chip object detection을 수행하여 2차 분류하도록 구성하였다. "
    "셋째, 기존 분류 체계에 등록되지 않은 상태에서 새롭게 발생하는 Unknown 불량을 검출하기 위해 SimCLR 계열 contrastive learning 기반 분석 구조를 구현하였다. "
...
KO_2_1 = (
    ...
    "생성된 Map은 중앙 저장소에 적재되어 분석 Web App[5]을 통해 대량 조회가 가능하며, "
    "이를 통해 엔지니어가 한 번에 수천 장 단위의 Map을 탐색·분석할 수 있는 운영 환경을 구축하였다."
)
...
KO_CONCLUSION = (
    ...
    "Cython 최적화와 32-color palette-indexed PNG로 Failbit Map의 대량 생성과 저장·운영을 가능하게 하였고, "
...
EN_1_2 = (
    ...
    "Second, even when maps were generated, defect judgment and type identification still depended on manual inspection by engineers, making full analysis difficult. "
    "To address these limitations, this paper proposes an integrated analysis architecture composed of a data pipeline for continuously generating, storing, and serving large volumes of Failbit Maps from raw data, a two-stage supervised framework for Known defects, and a self-supervised framework for Unknown defects. "
    "The main contributions are as follows."
)

EN_1_3 = (
    "First, we implemented a data-processing foundation capable of real-time ingestion of large equipment logs and hourly Failbit Map generation, enabling sustained large-scale map creation and operational use. "
    "Second, for Known defects, we used ConvNeXt V2 to classify wafer-level defect-chip distribution patterns in the first stage, and then applied ROI-based YOLO to perform chip-level object detection on samples prone to confusion due to similar distribution patterns, as a second-stage refinement. "
    "Third, for newly emerging defects not registered in the existing taxonomy, we implemented a SimCLR-family contrastive-learning framework for Unknown defect discovery. "
    "In addition, grid-structured local sampling was introduced to encode spatial occurrence information, enabling more precise Unknown defect detection."
)
...
EN_2_1 = (
    ...
    "the former was accelerated about 100x with Cython (Fig. 1), and the latter was reduced by about 75% with a 32-color palette-indexed PNG (Fig. 2). "
    "Generated maps are stored in a central repository and served through the analysis web application[5], "
    "enabling engineers to browse and analyze thousands of maps at a time at scales that were previously infeasible."
)
...
EN_2_2_B = (
    ...
    "To compensate for this, the second stage was configured so that ROI-based YOLO further inspects low-confidence samples and evaluates defect-chip morphology and appearance patterns within the ROI (Fig. 3)."
)
...
EN_2_2_C = (
    ...
    "This choice reflects the operational environment in which more than 20,000 wafer Failbit Maps are generated per day across all products. "
    "Optuna was then used to optimize fine-tuning hyperparameters such as learning rate, weight decay, focal loss, and class weight, improving weighted F1 to 0.92, "
    "and ROI-YOLO refinement further increased the final score to 0.95."
)

EN_2_3_A = (
    "Unknown defect discovery was defined as the task of grouping similar patterns and identifying candidate defect groups. "
...
EN_2_3_B = (
    ...
    "The remaining 6 groups were interpreted as lot-specific warning-level noise patterns or patterns that did not lead to actual chip defects."
)

EN_CONCLUSION = (
    "This study implemented an integrated architecture for full Failbit Map generation and automated defect analysis. "
...
if __name__ == "__main__":
    doc_ko = _build("ko")
    out_ko = OUT_DIR / "paper_codex_2page_rev160.docx"
    doc_ko.save(str(out_ko))
    print(f"saved: {out_ko}")

    doc_en = _build("en")
    out_en = OUT_DIR / "paper_codex_2page_rev160_en.docx"
