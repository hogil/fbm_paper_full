...
The data pipeline was designed to ingest equipment Raw Logs in real time and generate Failbit Maps on an hourly cycle. The main bottlenecks were the hex-to-grade conversion speed for roughly 10 million encoded test values per wafer and the image storage size; the former was accelerated about 100x with Cython (Fig. 1), and the latter was reduced by about 75% with a 32-color palette-indexed PNG (Fig. 2). Generated maps are stored in a central repository and served through the analysis web application[1], enabling engineers to browse and analyze thousands of maps at a time in an operational setting.
...
Unknown defect discovery was defined as the task of grouping similar patterns and identifying candidate defect groups. Using 10,000 production maps from five days, a SimCLR[4]-family model was trained with contrastive learning using grid-structured local sampling to reflect the zone-based nature of wafer defect interpretation. HDBSCAN[5] was then applied to a separate set of 2,000 maps from one additional day to group similar patterns.
