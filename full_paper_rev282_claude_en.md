...
**Abstract** - **A Failbit Map is ultra-high-resolution data generated during semiconductor Electrical Die Sorting (EDS) testing, with about 10 million pixels per wafer, and is a key source for failure-pattern analysis. In practice, however, large-scale Failbit Map retrieval was difficult, and the few retrievable maps were analyzed manually by engineers. To address this limitation, we built a large-scale Failbit Map data pipeline. Known failures were addressed with 2-stage supervised classification. Unknown failure candidate groups were derived from Failbit Map similarity using self-supervised embeddings and HDBSCAN. Conversion speed improved by about 100x with Cython, and storing images as 8-bit palette-indexed PNG instead of 24-bit RGB reduced the stored file size by about 80%. Known failure classification combined ConvNeXtV2 first-stage classification with Region-of-Interest YOLO (ROI-YOLO) rechecking for low-confidence samples, reaching a weighted F1 score of 0.95. For Unknown candidate discovery, a grid-level local loss was added to the contrastive-learning (InfoNCE) embedding so that small failure regions would not be diluted in the whole-wafer representation. When applied to a separate one-day set of 2,000 real wafer images, the method produced 13 Unknown failure candidate groups, and yield engineers confirmed 7 of them as actual failure groups. Some chips contain two or more overlapping failures, which a single-failure classification approach cannot separate well. To address this, we added a chip multi-label classification structure. Because overlapping-failure labels were hard to collect in sufficient quantity, single-failure chip labels were used as the training source. FCM-PM (Full-Cover CutMix with Pair Mask) achieved a bit-F1 score of 0.9927 and a False Alarm Rate (FAR) of 0.00% on a separate 2-combo synthesis evaluation set.**
...
In the previous analysis environment, only a few Failbit Maps could be retrieved at once, and storing large image volumes long-term was difficult, so image-based analysis could not be routinely used. Engineers checked failure status from chip-level measure values and classified recurring failures using formula- and threshold-based rules. This approach was fast and well-founded, but it depended on the rules each engineer defined. Chip-level numbers alone also make it hard to see the relation to neighboring chips, the location within the wafer, and the failure shape as a whole. Without this spatial information, it is difficult to decide which process condition to check first. An edge-ring, center-cluster, or directional pattern can each point to a different cause, which is why the Failbit Map must be examined. For a wafer classified as a failure, engineers used the Failbit Map and Transmission Electron Microscopy (TEM) to section and inspect samples, examined features such as Critical Dimension (CD) shifts or misalignment, and interpreted the process structure and layout cause. The problem was that large numbers of Failbit Maps could not be generated quickly, and the needed wafers could not be retrieved immediately.
...
| Web viewer | Search, overlay, measurement view | Engineer inspection screen |
...
In real data, new failures outside the existing 16 classes appear rarely, and ground-truth labels are almost absent. Supervised contrastive learning (SupCon [5]) risks absorbing a new pattern into an existing class, so we trained a contrastive embedding that does not depend on labels. The initial implementation used InfoNCE loss and a MoCo (Momentum Contrast) [18] queue of size 4096 to compensate for the scarcity of negatives in small batches.

The learned embedding is clustered with HDBSCAN. The HDBSCAN clustering result is shown as candidate groups in the web viewer so that engineers can inspect new failure candidates first. The final development configuration uses a global/local contrastive loss with the queue size extended to 16K, and a noise wafer is included in a group only when nearest-neighbor voting exceeds the threshold. Because missing a failure is more critical than a false alarm, we prioritized reducing missed failures.
...
| Unknown | 13 candidate groups / 7 actual failure groups | Engineering confirmation |
...
| Checked items | Map and measurement values checked separately | Overlay, composite, and chip measurement values checked together |
| New failure management | Records kept by each engineer | Registration history checked in the web viewer |
...
Future work will expand the product scope and the analysis granularity. Flash YE has also requested a feasibility review for applying this flow to bin-map and chip multi-label analysis, and any actual deployment will proceed as a separate task after checking data access rights and product differences. On the cause-analysis side, we will reinforce the function that shows wafer images and chip-level measurement values on one screen and records the supporting evidence. The contrastive embedding can be extended into a search index that finds similar past cases and engineer records. Any large-language-model-based function that summarizes image, trend, and history information will remain an assistant feature and will require engineer approval and history logging.

## 3. Conclusion

This work treats the Failbit Map not as a one-time image for lookup, but as part of an analysis flow that leads to engineer confirmation and training-data accumulation. With Cython conversion and palette PNG storage, Failbit Maps are generated every hour at scale, and the map and chip-level measurement values can be reviewed on the same screen. Known failure reached a weighted F1 score of 0.95 by rechecking only low-confidence samples with ROI-YOLO. Unknown failure candidate groups were derived using self-supervised embeddings and HDBSCAN, and yield engineers confirmed 7 of the 13 candidate groups as actual failure groups. The chip multi-label structure also addressed the problem that overlapping-failure chips cannot be classified by a single-failure analysis approach, and FCM-PM reached a bit-F1 score of 0.9927 and FAR 0.00% on a 2-combo synthesis evaluation set. The core contribution is not the performance of any single model, but the implementation of an operational analysis workflow that connects Failbit Map generation, engineering confirmation, and training-data accumulation.
