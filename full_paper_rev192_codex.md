...
**Abstract** - **Failbit Map은 wafer 위 failure 위치와 grade를 함께 남기는 진단 자료다. 그러나 실제 분석에서는 chip별 measure value와 제한된 map 조회가 따로 움직이는 경우가 많아, 같은 형태가 반복되는지 보거나 신규 failure 후보를 빠르게 좁히기 어렵다. 이 작업에서는 EDS(Electrical Die Sorting) raw log에서 만든 Failbit Map, chip 좌표, chip measure, 모델 출력, review label을 같은 wafer 좌표에 맞췄다. 이 자료 위에서 Known fail은 ConvNeXtV2-Base와 저신뢰 표본 한정 ROI(Region of Interest)-YOLO(You Only Look Once) 2-stage로 field validation weighted F1 0.95를 확인했다. Unknown fail은 대조학습 embedding과 HDBSCAN(Hierarchical Density-Based Spatial Clustering of Applications with Noise)으로 운영 wafer를 13개 후보 묶음으로 줄였고, 현업 검토에서 7개 신규 failure group을 확인했다. chip 수준에서는 object-id map을 생성 데이터 기준 후속 개발 항목으로 두고, FCM-PM(Full-Cover CutMix + Pair Mask)으로 single-failure source만 이용한 2-combo 평가에서 micro-F1(bit-F1) 0.9927과 FAR 0.00%를 얻었다. 각 결과는 나온 조건이 다르므로 field validation, field review, generated development, controlled synthetic benchmark로 나누어 해석했다. 따라서 Failbit Map은 한 번 보고 끝나는 그림이 아니라, wafer ID로 다시 찾고 chip overlay를 확인하며 라벨을 다음 학습에 쌓을 수 있는 자료가 된다.**
...
![DRAM cell에서 좌표 보존 Failbit Map까지](figures/full_paper_rev192/fig01_dram_to_fbm.png)
...
출발점은 현업에서 반복해서 부딪힌 두 문제였다. Failbit Map을 대량으로 만들고 볼 수 있어야 했고, 사람이 정한 임계값만으로는 신규 pattern을 놓칠 수 있었다. 실제 작업은 raw log를 palette PNG와 chip annotation으로 바꾸는 데서 시작했다. FTN, QTN, BIN measure는 같은 chip 좌표에 붙이고, 모델 출력과 review label도 wafer ID와 chip 위치로 다시 찾을 수 있게 했다. 이 자료 위에서 등록된 Known failure를 판정하고, label이 없는 Unknown failure는 담당자가 볼 후보 묶음으로 줄였다. chip 수준에서는 이미 알고 있는 chip 좌표를 쓰는 object-id map과 single 불량 chip만으로 2-combo를 학습하는 FCM-PM을 별도 경로로 두었다. 현업 review 화면에서는 검색, map view, chip overlay, measure 확인, label 관리가 이어진다. 운영에서 나온 숫자와 모델 검증값은 나온 자리부터 다르다. 그래서 Known F1, Unknown 13->7, object-id map, FCM-PM, 운영 효과는 Table 3에서 출처를 따로 표시했고 하나의 metric처럼 합산하지 않았다.

여기서 계속 붙잡은 기준은 모델 개수가 아니라 좌표였다. 이미지, chip 좌표, 계측값, review 결과가 같은 wafer 좌표에 남아야 담당자가 한 wafer를 다시 열었을 때 같은 chip을 볼 수 있다. 그래야 Known 판정 결과, Unknown 후보 묶음, chip-level 검증 결과가 서로 다른 파일에 흩어지지 않는다. categorical object-id를 자연 해상도 one-hot으로 인코딩하면 fixed chip coordinate라는 반도체 prior 덕분에 localization 부담이 줄고, class별 공간 패턴이 남는다(측정 근거는 2.2.1). 이 결과는 모델 용량 우열이 아니라 입력 표현의 효과로 해석한다. 정답 2-combo 라벨이 없는 조건에서는 FCM-PM으로 단일 불량 chip에서 중복 불량 학습 신호를 만들었다. Pair Mask가 합성 background의 false alarm을 줄이는 원인이라는 점은 ablation으로 확인하였다(2.2.3). object-id map은 양산 적용 성과가 아니라 생성 데이터 기준 후속 개발 결과다(2.2.1).

### 1.1. 기술 개요

설계할 때 가장 먼저 둔 조건은 웨이퍼의 공간 의미를 손상시키지 않는 것이었다. 그래서 저장은 RGB가 아니라 palette로 했고, 방향 의미가 바뀌는 flip 증강은 쓰지 않았다. backbone은 국소 수용장과 선택적 2-stage 검증을 기준으로 골랐다. Unknown 학습에서는 random crop 대신 격자를 고정했고, object-id 입력은 정수 보간 대신 categorical one-hot으로 두었다. Table 1에는 이 선택들이 실제로 어디에 쓰였는지만 짧게 모았다. palette PNG는 map을 저장하고, fixed chip crop은 chip 단위 판단을 만들며, web 화면은 같은 좌표의 map과 measure, label을 같이 보여 준다. Unknown 경로의 InfoNCE(Information Noise-Contrastive Estimation)는 라벨 없이 같은 wafer는 가깝게 다른 wafer는 멀게 임베딩을 학습하는 대조학습 목적함수이고, HDBSCAN[7]은 밀도 군집과 noise를 함께 분리하는 군집화 방법이다.
...
![좌표 보존 분석 파이프라인](figures/full_paper_rev192/fig02_pipeline_architecture.png)
...
![ROI-YOLO 2-stage 검출](figures/full_paper_rev192/fig03_roi_yolo.png)
...
![object-id map 생성 흐름](figures/full_paper_rev192/fig04_objid_flow.png)
...
![chip 라벨 공간 예시](figures/full_paper_rev192/fig05_chip_label_space.png)
...
![FCM-PM augmentation](figures/full_paper_rev192/fig06_fcm_pm.png)
...
![val_margin selection and NB-reject sidecar](figures/full_paper_rev192/fig07_probability_control.png)
...
결과를 보기 전에 숫자가 나온 자리를 먼저 적었다. Known F1은 field validation 값이고, Unknown 13->7은 담당자가 볼 후보를 줄인 field review 결과다. object-id map은 생성 데이터 기준 후속 개발값이며, FCM-PM은 통제된 합성 평가값이다. viewer/data pipeline의 운영 효과도 모델 benchmark metric과 성격이 다르다. 그래서 Table 3에 각 수치의 출처를 먼저 표시하고, 운영 효과와 모델 metric은 합산하지 않았다.
...
![raw와 object-id map 구분력 비교](figures/full_paper_rev192/fig_objid_compare.png)
...
![운영 web app(map viewer) 화면](figures/full_paper_rev192/fig09_webapp_viewer.png)
