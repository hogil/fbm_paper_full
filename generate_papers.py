...
ABSTRACT_CODEX = (
    "Failbit Map은 반도체 EDS Test에서 생성되는 웨이퍼당 약 1,000만 point 규모의 초고밀도 데이터로, 수율 개선을 위한 공간적 불량 패턴 해석의 핵심 자료이다. "
    "그러나 기존 시스템은 호출 기반으로 소수의 Map만 조회할 수 있어, 하루 수천 장 규모의 대량 분석을 위해서는 raw data를 별도로 수집해 Failbit Map과 메타데이터로 재구성하는 과정이 먼저 필요했다. "
    "그 결과 실제 현업에서는 Failbit Map 대신 chip별 Measure 평균 기반 분석에 주로 의존해 왔으며, 등록 불량의 세밀한 분류와 미등록 Unknown 불량 후보 발굴을 동시에 수행하기 어려웠다. "
    "본 논문은 이를 해결하기 위해 DRAM Failbit Map 기반 Known·Unknown 불량 분석 AI 아키텍처를 제안한다. Known 경로에서는 ConvNeXtV2-Base를 주 분류기로 사용하고, "
    "wafer 내 발생 영역이 유사하여 혼동이 집중되는 저신뢰 샘플에 한해 Grad-CAM ROI 기반 YOLO를 선택적으로 적용하는 2단계 구조를 설계하여 16-class weighted F1을 0.78에서 0.95까지 향상시켰다. "
    "Unknown 경로에서는 SimCLR 계열 contrastive learning, 6×6 structured local sampling, HDBSCAN 군집화를 결합하여 5일치 운영 이미지 약 10,000장으로 표현 공간을 학습하고 별도 1일치 2,000장을 13개 후보 그룹으로 압축하였다. "
    "이는 직접 검토해야 할 대상을 약 154배 줄인 결과이며, 13개 그룹 모두 설명 가능한 패턴으로 정리되었다. 이 중 7개는 실제 의미 있는 불량 그룹으로 확인되었고, 나머지 후보 역시 1 lot 편중 패턴 또는 현업 불량으로 이어지지 않은 이상 이미지로 정리되어 후속 검토 리스트로 활용되었다. "
    "또한 raw data 재구성, palette-indexed PNG 표현, 고속 UI, 후속 원인 분석용 Measure 연계를 결합함으로써, 제안 아키텍처가 실제 대량 불량 분석 환경에서 동작할 수 있음을 확인하였다."
)
...
def build_2page_claude() -> Document:
    ...
    add_body(doc,
        ...
        "수율 개선의 핵심 자료이다. "
        "그러나 대량 데이터 수집·저장 인프라가 구축되어 있지 않고, "
        "기존 열람 시스템으로는 최대 48매 수준만 확인할 수 있어 "
        "하루 수천 장씩 발생하는 Failbit Map을 전수 분석하기 어렵다. "
        "이로 인해 현업에서는 Failbit Map 대신 "
        "chip별 Measure 데이터를 wafer 단위로 집계하는 방식에 주로 의존해 왔으며, "
        "이 방식으로는 공간적 기하 형태를 갖는 불량 패턴을 파악할 수 없다.",
        indent=True, space_after=Pt(2))
    add_body(doc,
        "기존 AI 연구는 공개 benchmark 기반 closed-set 분류에 집중되어 있어 "
        "실제 운영 환경의 Unknown 불량에 대응하지 못한다. "
        "Nakazawa 등 [1]과 Alawieh 등 [2]은 CNN 기반 분류를 체계화하였으나 "
        "Unknown 불량 탐지가 불가능하며, "
        "Jang 등 [4]의 Unknown 탐지 연구도 라벨 없는 실제 운영 이미지에 대한 검증이 부재하다.",
        indent=True, space_after=Pt(2))
    add_body(doc,
        "본 연구의 기여는 다음과 같다. "
        "첫째, ConvNeXtV2와 선택적 ROI-YOLO를 결합한 2단계 구조로 "
        "16-class Known 불량 분류 F1 0.95를 달성하였다. "
        "둘째, contrastive learning과 HDBSCAN을 결합하여 "
        "라벨 없는 운영 이미지에서 Unknown 불량 후보를 그룹 단위로 자동 발굴하는 체계를 현업에 최초로 구현하였다. "
        "셋째, 설비 로그 기반 raw data 대량 수집 파이프라인·수만 장 탐색 UI·Measure 연계를 통합하여 "
        "Failbit Map을 실제 현업 분석에 활용할 수 있는 운영 체계를 구축하였으며, "
        "현재 DRAM 생산 라인에 배포 운영 중이다.",
        indent=True, space_after=Pt(2))
