...
TABLE_PERF_CLAUDE = {
    "caption": "Table 1. 등록 불량 Backbone 및 개선 단계 (16-class, test Weighted F1)",
...
def build_codex_revised() -> Document:
    ...
    add_body(doc,
        ...
        "여전히 엔지니어의 직접 확인에 의존하고 있어 전수 분석이 어렵다. 따라서 현업 적용을 위해서는 대량 Raw "
        "Data를 처리하는 데이터 파이프라인과 Failbit Map 기반 Known·Unknown 불량 분석 AI가 함께 필요하다.",
        indent=True, space_after=Pt(2))
    add_body(doc,
        "본 논문의 주요 기여는 다음과 같다.",
        indent=True, space_after=Pt(2))
    ...
    add_body(doc,
        "Known 불량 분석은 16개 기등록 클래스를 대상으로 하며, 총 1,500개의 라벨된 Failbit Map을 사용하였다. "
        "1단계는 ConvNeXtV2 기반 wafer-level 분류기로 구성하고, 저신뢰 샘플에 한해 2단계 ROI 기반 YOLO를 적용하였다.",
        space_after=Pt(2))
    add_body(doc,
        "특히 혼동되는 클래스는 wafer 내 defect chip distribution은 유사하지만 defect chip 내부 pixel morphology가 다르므로, "
        "wafer 전체 이미지를 보는 CNN만으로는 오분류가 발생할 수 있다. 이에 2단계 ROI-YOLO가 defect chip 내부 pattern을 추가로 판별하도록 구성하였다(Fig. 3).",
        space_after=Pt(2))
    ...
    add_body(doc,
        "Unknown 불량은 후보 grouping 문제로 정의하였다. "
        "5일치 운영 데이터 약 10,000장으로 SimCLR 계열 contrastive learning 기반 임베딩을 학습하고, 별도 1일치 2,000장에 HDBSCAN을 적용하여 유사 패턴을 그룹화하였다. "
    ...
    add_heading(doc, "3. 실험 결과", level=1)
    add_body(doc,
        "데이터 계층에서는 Cython 최적화로 hex-to-grade 변환 속도를 약 100배 향상시켰고, palette-indexed PNG 적용으로 이미지 용량을 약 75% 절감하였다. "
        "이를 통해 1시간 주기의 Failbit Map 생성을 안정적으로 수행할 수 있었다.",
        space_after=Pt(2))
    add_table(doc, TABLE_PERF_CLAUDE)
    add_body(doc,
        "전체 운영 기준 하루 약 2만 장 이상의 Wafer가 1시간 주기로 처리되므로, backbone 선택에서는 성능과 처리량을 함께 고려하였다. "
        "MaxViT와 ConvNeXtV2 (Ref)는 동일한 test Weighted F1 0.87을 보였으나, ConvNeXtV2가 처리량 측면에서 더 유리하였다. "
        "Baseline CNN의 test Weighted F1 0.78은 ConvNeXtV2 (Ref)로 0.87, Ref + Optuna로 0.92, Ref + Optuna + ROI로 0.95까지 개선되었다. "
        "이는 backbone 교체, HPO, ROI refinement가 단계적으로 누적 기여했음을 보여준다.",
        space_after=Pt(2))
    add_body(doc,
        "Unknown 경로에서는 운영 이미지 2,000장에 대한 grouping 결과 13개 후보 그룹이 검출되었으며, 현업 분석 엔지니어 검증 결과 이 중 7개가 실제 불량 그룹으로 판정되었다. "
        "이는 미등록 불량을 실제 운영 데이터에서 직접 검출할 수 있음을 보여준다.",
        space_after=Pt(2))

    add_heading(doc, "4. 결론", level=1)
    add_body(doc,
        "본 연구는 대량 Failbit Map 생성 파이프라인과 Known·Unknown 불량 분석 AI를 통합 구현하였다. "
        "현재 DRAM 생산 라인에서 운영 중이며, 1시간 주기 Map 생성과 등록·미등록 불량 자동 분석을 통해 FAB 품질 분석과 신규 불량 대응에 실질적으로 기여하고 있다. "
        "향후 전 제품군 확산과 함께 FTN·QTN과 같은 전기적 측정값을 Failbit Map과 통합하여, 공간 패턴과 측정 신호를 함께 반영하는 multimodal 불량 분석 구조로 확장할 계획이다.",
        indent=True, space_after=Pt(2))
