...
ABSTRACT = (
    "반도체 제조 공정에서 Failbit Map 기반 웨이퍼 불량 분석은 수율 관리와 이상 원인 추적의 "
    "핵심 업무이지만, 실무 환경에서는 이를 체계적으로 수행하기 어렵다. "
    "수치 계측값만으로는 불량의 공간적 패턴을 파악할 수 없고, 그렇다고 발생하는 wafer 수에 비해 "
    "맵 이미지를 대량으로 열람·검토하는 수단도 없다. 현재 실무자들은 관심 있는 wafer의 맵 파일을 "
    "개별 저장하고 메신저·이메일로 공유하는 비공식 방식에 의존하며, 분석 결과가 개인에게 귀속되어 "
    "조직 차원의 지식으로 축적되지 않는 구조적 문제가 있다. 이 상황에서 등록된 불량의 자동 분류만으로는 "
    "충분하지 않으며, 라벨 등록 지연과 라벨 노이즈, 신규 공정 변화에 따른 unknown defect 발생, "
    "이미지와 칩 계측값의 동시 해석 요구가 추가적으로 존재한다. "
    "본 연구에서는 이러한 문제를 해결하기 위해 데이터 생성, 등록 불량 분류, 미등록 불량 군집화, "
    "사용자 검토를 하나의 흐름으로 연결하는 도메인 지식 기반 통합 파이프라인을 제안하였다. "
    "데이터 계층에서는 dual-bucket 로그 수집, Cython 기반 고속 파싱, palette-indexed PNG 및 "
    "positions JSON 생성을 통해 웨이퍼 이미지와 chip-level 좌표 및 계측값을 정합하였다. "
    "생성되는 wafer map은 자연영상과 달리 검사 결과를 의미하는 약 20개 내외의 이산 색상만 사용하므로, "
    "32-color palette PNG로 무손실 의미 보존과 효율적 저장이 가능하였다. "
    "등록 불량 경로에서는 16개 클래스에 대해 ConvNeXtV2 기반 백본 선택과 Optuna 기반 학습 최적화를 통해 "
    "weighted F1을 0.78에서 0.92까지 향상시켰고, 이후 difficult class 및 low-confidence sample에 "
    "선택적으로 Grad-CAM 기반 ROI 추출, 클래스별 spatial prior, YOLO 이차 검증을 적용하는 2단계 구조로 "
    "해당 케이스의 교정 정밀도 90% 이상을 확보하며 최종 0.95를 달성하였다. "
    "미등록 불량 경로에서는 InfoNCE 기반 Contrastive Learning과 HDBSCAN을 적용하여 "
    "unknown defect 후보를 자동 군집화하고, 전문가 검토와 신규 라벨 등록을 지원하였다."
)
