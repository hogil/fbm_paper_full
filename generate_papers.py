...
TITLE_KO_CODEX = "DRAM Failbit Map 기반 Known·Unknown 불량 분석 AI 아키텍처"
TITLE_EN_CODEX = ("An AI Architecture for Known/Unknown Defect Analysis "
                  "Based on DRAM Failbit Maps")
...
ABSTRACT_CLAUDE = (
    "Failbit Map은 반도체 EDS(Electrical Die Sorting) Test에서 생성되는 웨이퍼당 약 1,000만 pixel 규모의 "
    "초고해상도 공간 데이터로, 불량의 위치·밀집도·형상·방향성을 정밀하게 보여주는 수율 저하 원인 분석의 핵심 자료이다. "
    "그러나 현업에서는 대량의 Failbit Map을 생성·저장·조회할 수 있는 환경이 부족하여(기존 분석 환경 기준 한 번에 48매 이하), "
    "불량 여부와 유형 판단이 엔지니어의 수작업 확인에 의존해 전수 분석이 어렵다. "
    "본 논문은 이를 해결하기 위해 Test Log를 변환하여 대규모 Failbit Map을 생성·저장하는 데이터 파이프라인과 "
    "Known·Unknown 불량을 통합 분석하는 AI 아키텍처를 End-to-End 구조로 구현하였다. "
    "파이프라인은 palette-indexed PNG로 이미지 용량을 약 75% 절감하고, 실시간 Log 수집과 1시간 주기 Map 생성을 통해 대량 운영 기반을 확보하였다. "
    "Known 경로는 ConvNeXtV2-Base를 주 분류기로 사용하고, Wafer 내 발생 영역이 유사하여 혼동이 집중되는 저신뢰 샘플에 한해 "
    "Grad-CAM ROI 기반 YOLO를 선택적으로 적용하는 2단계 구조로 설계하여 16-class weighted F1을 baseline 0.78에서 0.95까지 향상시켰다. "
    "Unknown 경로는 레이블 없이 SimCLR 계열 contrastive learning으로 표현 공간을 학습하고, "
    "Wafer의 zone 기반 불량 해석 특성을 반영하기 위해 grid structured local sampling을 적용하였다. "
    "양산 5일치 약 10,000장으로 표현 공간을 학습한 뒤 별도 1일치 2,000장에 대해 추론한 결과 13개 후보 그룹으로 압축되었고(검토 단위 약 154배 축소), "
    "현업 엔지니어 검증을 통해 이 중 7개가 실제 의미 있는 불량 그룹으로 확인되었다. "
    "나머지 후보 역시 1 lot 편중 패턴 또는 이상 이미지로 정리되어 후속 검토 리스트로 활용되었다. "
    "본 연구는 제안 아키텍처가 실제 대량 불량 분석 환경에서 운용 가능함을 확인하였다."
)

ABSTRACT_CODEX = (
    "Failbit Map은 반도체 EDS Test에서 생성되는 웨이퍼당 약 1,000만 pixel 수준의 초고해상도 공간 데이터로, 불량의 위치, 밀집도, 형상, 방향성을 정밀하게 보여주는 핵심 자료이다. "
    "그러나 실제 현업에서는 대량의 Failbit Map을 생성·조회하고 전수 분석할 수 있는 환경이 부족하여, 엔지니어의 수작업 확인에 의존한 분석이 이루어지고 있다. "
    "본 논문은 이를 해결하기 위해 Test Log를 변환하여 대규모 Failbit Map을 생성·저장하는 데이터 파이프라인과, Known 불량 및 Unknown 불량을 분석하는 AI 불량 분석 구조를 구현하였다. "
    "Known 불량 분류는 ConvNeXtV2-Base를 주 모델로 사용하고, 신뢰도가 낮은 샘플에 한해 ROI 기반 YOLO를 선택적으로 적용하는 2단계 구조로 설계하였으며, 이를 통해 F1-score 0.95를 달성하였다. "
    "Unknown 불량 검출은 레이블 없이 SimCLR 계열 contrastive learning을 사용하였고, wafer의 zone 기반 불량 해석 특성을 반영하기 위해 grid structured local sampling을 적용하였다. "
    "실제 양산 5일치 Failbit Map 10,000장을 학습한 뒤 1일치 2,000장에 대해 추론한 결과 13개 불량 그룹이 검출되었으며, 현업 분석 엔지니어의 검증 결과 이 중 7개가 실제 의미 있는 불량 그룹으로 확인되었다. "
    "이를 통해 레이블이 없는 실제 운영 이미지에서도 Unknown 불량을 그룹 수준에서 검출하고, 전문가 검증을 통해 실제 불량으로 식별할 수 있음을 확인하였다."
)
...
def build_2page_claude() -> Document:
    ...
    add_body(doc,
        ...
        "Grade 0(정상)부터 7(최대 불량)까지로 표현한 데이터이다. "
        "Wafer 1장에는 약 1,000만 개의 block이 포함되므로, Failbit Map은 불량의 위치, 밀집도, 형상, 방향성을 "
        "반영하는 초고밀도 공간 데이터이자 수율 저하 원인 분석의 핵심 데이터이다. "
        "그러나 현업 분석은 주로 Chip 단위 Measure(측정 항목별 집계값)의 발생 빈도를 기준으로 이상 여부를 판단하고 있어, "
        "Failbit Map 전체에서만 드러나는 불량 분포의 공간 패턴을 충분히 포착하지 못하며, "
        "이로 인해 수율 저하 원인 추적에 사각지대가 발생할 수 있다. "
        "따라서 수율 저하 원인을 정밀하게 분석하기 위해서는 Measure 기반 접근만으로는 충분하지 않으며, "
        "Failbit Map 자체를 핵심 분석 단위로 삼아야 한다.",
        indent=True, space_after=Pt(2))
    add_body(doc,
        "실제 현업 적용에는 두 가지 제약이 있다. 첫째, 대량 Map 생성이 어렵다. "
        "설비 Log는 Wafer당 10~50MB 수준이며 특정 제품군에서는 하루 약 2,000장의 Wafer가 발생하지만, "
        "기존 분석 환경에서는 속도와 메모리 제약으로 한 번에 48매 이하만 처리할 수 있다. "
        "둘째, Map이 생성되더라도 불량 여부와 유형 판단은 여전히 엔지니어의 직접 확인에 의존하고 있어 전수 분석이 어렵다. "
        "두 제약은 독립적이지 않다. 파이프라인이 없으면 AI 학습·운영에 필요한 입력 데이터를 확보할 수 없고, "
        "AI가 없으면 생성된 Map 역시 수작업 판정 병목을 해소하지 못한다. "
        "본 논문은 대량 Raw Data를 처리하는 데이터 파이프라인과 Failbit Map 기반 Known 불량 분류 및 "
        "Unknown 불량 검출을 통합한 AI 아키텍처를 하나의 End-to-End 분석 구조로 구현하여 실데이터에 적용·검증하였다.",
        indent=True, space_after=Pt(2))
    add_body(doc,
        "본 논문의 주요 기여는 다음과 같다. "
        "첫째, Test 부서와 협의하여 대량 Raw Data 적재 체계와 전수 Failbit Map 생성 파이프라인을 구축하고, "
        "palette-indexed PNG로 이미지 용량을 약 75% 절감하여 실시간 Log 수집부터 "
        "Failbit Map 생성·저장·조회, AI 분석까지 연결되는 End-to-End 운영 기반을 마련하였다. "
        "둘째, Known 불량 정밀 분류를 위해 ConvNeXt V2와 ROI 기반 YOLO를 결합한 2단계 구조를 제안하여, "
        "ConvNeXt V2로 Wafer 전역 패턴을 우선 분류하고 신뢰도가 낮은 샘플에 한해 ROI 기반 YOLO로 불량 영역의 위치 정보를 추가 반영함으로써 "
        "전역 패턴과 국소 위치 정보를 함께 활용하는 정밀 분류를 구현하였다. "
        "셋째, 기존 분류 체계에 등록되지 않은 상태에서 새롭게 발생하는 Unknown 불량을 검출하기 위해 "
        "SimCLR 계열 contrastive learning 기반 분석 구조를 구현하고, Wafer를 격자 단위 국소 영역으로 나누어 "
        "특징을 학습하는 grid structured local sampling을 적용하여 동일한 형태의 불량이라도 발생 위치에 따라 "
        "구분될 수 있도록 함으로써, Unknown 불량을 유사 패턴 그룹 수준에서 검출·해석할 수 있도록 하였다.",
        indent=True, space_after=Pt(2))
    ...
    add_body(doc,
        "AI 불량 분석을 위해서는 수만 장 규모의 Failbit Map 데이터셋이 필요하나, "
        "기존에는 raw data가 대량으로 수집·축적되어 있지 않아 데이터 자체를 확보할 수 없었다. "
        "이를 해결하기 위해 Failbit Map 설비 로그(Bucket A)에서 raw data를 "
        "자동으로 대량 수집·적재하는 전용 파이프라인을 구축하였다. "
        "수집된 데이터는 Cython 기반 hex 파싱으로 chip tile array를 복원하고, "
        "Failbit Map이 grade 0~7의 제한된 이산 색상만을 사용하는 특성을 활용하여 "
        "32-color palette-indexed PNG로 저장함으로써 RGB 대비 약 75%를 절감하면서 무손실을 유지한다. "
        "이렇게 생성된 이미지가 Known·Unknown AI 분석과 UI의 공통 입력이 된다. "
        "전체 파이프라인은 서버에서 1시간 주기로 자동 실행되며, "
        "Measure 로그(Bucket B)와 wafer ID 기준으로 매칭하여 "
        "chip별 FTN·QTN·BIN을 annotation 파일에 사전 적재한다.",
        space_after=Pt(2))

    add_subheading(doc, "2.2 Known 불량 분류 (16-class)")
    add_body(doc,
        "파이프라인으로 수집된 Failbit Map 이미지에 엔지니어가 불량 유형을 라벨링하여 구성한 "
        "약 1,500장(train/test 4:1 층화 분할)을 학습 데이터로 사용하였다. "
        "ConvNeXtV2-Base [5](FCMAE+IN-22k, 384×384)를 "
    ...
    add_body(doc,
        "공정 변화 시 발생하는 Unknown 불량은 사전에 클래스를 정의할 수 없어 "
        "supervised 방법으로는 탐지 불가능하므로, 본 연구에서는 이를 후보 grouping 문제로 정의하였다. "
        ...
        "결과는 (1) 하루 2,000장을 13개 그룹으로 압축한 검토 단위 축소 효과(Fig. 2), "
        "(2) 각 그룹이 실제 불량·1 lot 헌팅·이미지 이상 중 하나로 설명 가능한 해석 가능성, "
        "(3) 실제 의미 있는 불량 그룹 발굴 수의 세 축으로 평가하였다. "
        "담당 엔지니어 검토 결과 13개 그룹 모두 설명 가능한 패턴으로 확인되었고, "
        "이 중 7개(53.8%)는 신규 불량, 나머지 6개는 1 lot 내 일시적 헌팅 또는 이미지 이상으로 식별되었다.",
        space_after=Pt(2))
    ...
    add_body(doc,
        ...
        "FTN·QTN은 공정 원인 분석에 필요한 chip 단위 전기 특성 데이터로, "
        "기존 20조 row 규모 DB 직접 조회의 지연·실패 없이 ms 수준으로 즉시 확인 가능하다. "
    ...
    add_body(doc,
        "Table 1은 Known 분류의 backbone 비교 및 단계별 성능을 보여준다. "
        "backbone 후보 중 ConvNeXtV2-Base가 MaxViT와 동등한 F1 0.87을 기록하면서 추론 속도가 더 빠르므로 "
        "최종 backbone으로 채택하였으며, Optuna HPO 적용으로 F1 0.92, "
        "저신뢰 샘플에 대한 선택적 ROI-YOLO 보정으로 최종 F1 0.95를 달성하였다. "
        "이는 wafer 전역 문맥을 빠르게 판별하는 CNN과 "
        "취약 샘플을 국부적으로 재확인하는 ROI-YOLO의 역할 분리가 유효함을 보여준다. "
        "Unknown 탐지에서는 하루 전체 Map 2,000장이 13개 후보 그룹으로 자동 압축되어(154배 축소) "
        "담당 엔지니어 검토 결과 7개(53.8%)는 신규 불량, 나머지 6개는 헌팅 또는 이미지 이상으로 정리되었다. "
        "Unknown 결과의 핵심은 단순 군집 정확도가 아니라, "
        "대량 운영 이미지에서 실제 검토 가치 있는 후보 리스트를 안정적으로 생성했다는 점이다.",
        space_after=Pt(2))
...
def build_codex_revised() -> Document:
    ...
    pk = doc.add_paragraph()
    rk = pk.add_run(
        "Keywords: Failbit Map, Known Defect Classification, Unknown Defect Discovery, "
        "ConvNeXtV2, ROI-YOLO, Contrastive Learning, HDBSCAN"
    )
    _apply_run_font(rk, size=PT9, italic=True)
    _set_single_spacing(pk)
    pk.paragraph_format.space_after = Pt(4)
    ...
    add_body(doc,
        "Failbit Map은 EDS(Electrical Die Sorting) Test에서 각 Memory Cell Block의 불량 cell 개수를 "
        "Grade 0(정상)부터 7(최대 불량)까지로 표현한 데이터이다. Wafer 1장에는 약 1,000만 개의 "
        "block이 포함되므로, Failbit Map은 불량의 위치, 밀집도, 형상, 방향성을 반영하는 초고밀도 공간 "
        "데이터이자 수율 저하 원인 분석의 핵심 데이터이다. 그러나 현업 분석은 주로 Chip 단위 "
        "Measure(측정 항목별 집계값)의 발생 빈도를 기준으로 이상 여부를 판단하고 있어, Failbit Map "
        "전체에서만 드러나는 불량 분포의 공간 패턴을 충분히 포착하지 못한다. 이로 인해 수율 저하 원인이 "
        "오분류되거나 미검출될 수 있다. 따라서 수율 저하 원인을 정밀하게 분석하기 위해서는 Measure 기반 "
        "접근만으로는 충분하지 않으며, Failbit Map 자체를 핵심 분석 단위로 삼아야 한다.",
        indent=True, space_after=Pt(2))
    add_body(doc,
    ...
    add_body(doc,
        ...
        indent=True, space_after=Pt(2))
    add_body(doc,
        "첫째, Test 부서와 협의하여 대량 Raw Data 적재 체계와 전수 Failbit Map 생성 파이프라인을 "
        "구축하였다. 또한 palette-indexed PNG를 적용해 이미지 용량을 약 75% 절감하고, 실시간 설비 Log "
        "수집과 1시간 주기 Failbit Map 생성 체계를 구현하여 대량 분석이 가능한 운영 기반을 마련하였다.",
        indent=True, space_after=Pt(2))
    add_body(doc,
    ...
    add_body(doc,
        ...
        "계열 contrastive learning 기반 분석 구조를 구현하였다. 또한 grid structured local sampling을 "
        "적용하여 동일한 형태의 불량이라도 발생 위치에 따라 구분될 수 있도록 하였으며, 이를 통해 Unknown "
        "불량을 유사 패턴 그룹 수준에서 검출하고 해석할 수 있도록 하였다.",
        indent=True, space_after=Pt(2))

    add_heading(doc, "2. 제안 방법", level=1)

    add_subheading(doc, "2.1 데이터 생성 및 표현 계층")
    add_body(doc,
        "제안 아키텍처의 기반 계층은 기존 시스템의 호출형 조회만으로는 Known·Unknown AI 분석을 대량 운영 데이터에 적용할 수 없다는 한계를 해결하기 위해, 설비 log 기반 raw data를 서버에서 1시간 주기로 자동 수집하고 이를 Failbit Map 이미지와 메타데이터로 재구성하여 분석과 UI에 동시에 활용할 수 있도록 적재하는 구조이다. "
        ...
        "현재 FTN·QTN은 모델 입력이 아니라 원인 해석과 공정 추적을 위한 분석 정보로 활용되며, 20조 row DB를 직접 반복 조회하던 구조 대신 사전 정합된 annotation으로 제공하여 ms급으로 즉시 확인할 수 있도록 하였다. 이러한 정합 구조는 향후 multimodal 분석의 기반으로도 활용될 수 있다.",
        space_after=Pt(2))
    ...
    add_subheading(doc, "2.3 Unknown 후보 발굴")
    add_body(doc,
    ...
    add_body(doc,
        ...
        "(1) 하루 2,000장의 검토 대상을 13개 그룹으로 압축한 검토 단위 축소 효과, "
        "(2) 각 그룹이 실제 불량, 1 lot 편중 패턴, 이상 이미지 중 하나로 설명 가능한 해석 가능성, "
    ...
    add_subheading(doc, "2.4 분석 UI와 후속 연계")
    add_body(doc,
        ...
        "즉, 본 시스템은 대량 이미지 탐색, 후보 선별, 원인 추론, 데이터 등록을 하나의 흐름으로 연결한 분석 체계이다.",
        space_after=Pt(2))
